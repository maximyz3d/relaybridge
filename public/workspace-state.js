(function(root, factory) { if (typeof module === 'object' && module.exports) module.exports = factory(); else root.WorkspaceState = factory(); })(typeof globalThis !== 'undefined' ? globalThis : this, function() {
  ...
  function currentWork(state) { const groups=taskGroups(state.tasks); if(state.thread?.queueTaskId) return {title:state.thread.role==='advisor'?'Getting a second perspective':'Codex is shaping the next step',state:state.thread.state,model:state.thread.model,progress:state.thread.progress}; return groups.active[0]||groups.blocked[0]||groups.queued[0]||null; }
