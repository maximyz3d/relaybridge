...
function checkGrounding({ prompt, cwd = null, cwdIdentityHash, seat, seatConfig = {}, dangerous = false,
  requiresWorkspaceAccess, inlineEvidence, cwdExplicit = typeof cwd === 'string' && !!cwd.trim() }) {
  ...
  const missingCwd = need.required && !cwdExplicit && !(evidence && !need.writeRequired && !dangerous);
  const allowed = !missingCwd && (need.writeRequired ? !!canWrite : !need.required || hasAccess || (!!evidence && !dangerous));
  return { version: 1, allowed, required: need.required, hasAccess, seat,
    ...
    ...(!allowed ? { reason: missingCwd ? 'Workspace inspection requires an explicit validated cwd; a default home directory does not identify the requested project.' : need.writeRequired
