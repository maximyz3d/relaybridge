...
// Absolute citations, which neither shape above can see: both require the token
// to start with a word character right after a whitespace/quote boundary, and a
// leading `/` (or a drive letter, whose `:` is not a boundary character either)
// never satisfies that. Agents cite files absolutely far more often than not —
// "I reviewed /home/u/proj/server.js" — so without these the post-hoc layer
// reported 'no-paths-cited' for the dominant citation style and never fired.
const ABS_POSIX_PATH_RE = /(?:^|[\s`'"(\[])(\/(?:[\w.-]+\/)*[\w.-]+\.[A-Za-z0-9]{1,8})(?=[\s`'")\],.:;]|$)/g;
const ABS_WINDOWS_PATH_RE = /(?:^|[\s`'"(\[])([A-Za-z]:[\\/](?:[\w.-]+[\\/])*[\w.-]+\.[A-Za-z0-9]{1,8})(?=[\s`'")\],.:;]|$)/g;
...
function extractReferencedPaths(text, limit = 60) {
  const out = new Set();
  const s = String(text || '');
  for (const re of [PATH_WITH_DIR_RE, BARE_FILE_RE, ABS_POSIX_PATH_RE, ABS_WINDOWS_PATH_RE]) {
    re.lastIndex = 0;
    let m;
...
function verifyReferencedPaths(output, cwd, { minPaths = 1 } = {}) {
  ...
  for (const rel of referenced) {
    // A citation in the other platform's flavour tells us nothing here: on
    // POSIX, path.resolve would glue `C:\repo\a.js` onto the root as one bogus
    // relative segment, so every Windows-style citation would read as a missing
    // file and manufacture the fabrication signal it is meant to detect.
    if (path.sep === '/' && (/^[A-Za-z]:[\\/]/.test(rel) || rel.includes('\\'))) continue;
  ...
  // Paths were cited but none of them were checkable here (all outside the
  // workspace, or in the other platform's flavour). Reporting 'ok' would be a
  // pass we did not earn; 'no-paths-cited' is the existing "learned nothing".
  let confidence = total === 0 ? 'no-paths-cited' : 'ok';
