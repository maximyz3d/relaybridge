...
class RunSupervisor {
  constructor(options = {}) {
    ...
    this.stopped = null;
    this.stall = null;
    // S4 (Refs #133): unverifiable CPU silence is not evidence of a wedged
    // run by itself -- it raises this incident marker once per streak but
    // is never enforced as a kill (see #checkIn / wedgedStreak).
    this.unsampledStall = null;
    // N-record-only (Refs #133): a would-be kill verdict reached only via a
    // post-exit evaluate()/markExited() call -- never one latched while the
    // process was alive. Informational, sticky, never surfaced as `stopped`.
    this.postExitSignal = null;
    // Sticky once markExited() is called: every subsequent evaluate() call
    // defaults latch to false, no matter which call site invokes it.
    this.exited = false;
  ...
  #checkIn(now, { latch = true } = {}) {
    ...
    if (progressed || productiveAssessment) {
      ...
      const silentCheckin = noNewBytes && noTokenGrowth;
      // S4 (Refs #133): only a CPU-confirmed-idle silent check-in feeds the
      // killable wedgedStreak. CPU silence that could not be sampled/verified
      // is tracked separately (unsampledWedgedStreak) and can never kill --
      // see the threshold check below, which only raises an incident marker.
      if (silentCheckin && cpuConfirmedIdle) {
        this.wedgedStreak += 1;
      } else {
        this.wedgedStreak = 0;
      }
      if (silentCheckin && cpuUnsampled && !cpuConfirmedIdle) {
        this.unsampledWedgedStreak += 1;
      } else {
        this.unsampledWedgedStreak = 0;
      }
    ...
    const fingerprint = {
      // N1 (Refs #133): the previous read (`this.cpuActiveSinceLast`, always
      // undefined) never carried the sampled CPU evidence. Read the field
      // this class actually maintains, cpuActiveSinceLastCheckIn, before it
      // is reset below for the next window.
      ...
      cpuActiveSinceLast: this.cpuActiveSinceLastCheckIn,
      totalTokens: tokens,
    ...
    if (this.wedgedStreak >= this.opts.wedgedCheckins) {
      return this.#stopOrNotify('wedged',
        `${this.wedgedStreak} consecutive check-ins with no new bytes, confirmed-idle CPU, no progress and no token growth`,
        now, { latch });
    }
    // S4 (Refs #133): unsampled CPU silence never kills. It raises an
    // incident marker (this.unsampledStall, sticky, surfaced by the caller)
    // once the wider unsampledWedgedCheckins threshold is reached, but the
    // run is only ever stopped through loop_confirmed, burn_without_progress
    // or assessor_stuck.
    if (this.unsampledWedgedStreak >= this.opts.unsampledWedgedCheckins && !this.unsampledStall) {
      this.unsampledStall = {
        reason: 'unsampled_wedged',
        detail: `${this.unsampledWedgedStreak} consecutive check-ins with no new bytes/tokens and unsampled CPU (never killed)`,
        at: now,
      };
    }
  ...
  // `latch: false` is the post-exit/record-only path (see markExited()):
  // check-ins and evidence still accumulate normally, but any verdict that
  // would otherwise kill is recorded as a non-kill observation instead of
  // ever setting `this.stopped`. A verdict latched earlier, while the
  // process was alive, is unaffected -- it already returned early above.
  evaluate(now = Date.now(), { latch = !this.exited } = {}) {
    ...
    if (this.opts.adaptive === true && this.assessmentEnabled
      ...
      const verdict = this.#stopOrNotify('assessor_stuck',
        'Current progress assessment and sustained repeated failures or repetition support stopping.', now, { latch });
      if (verdict) return verdict;
    }

    if (this.#checkInDue(now)) {
      const verdict = this.#checkIn(now, { latch });
      if (verdict) return verdict;
    }
  ...
  // N-record-only (Refs #133): the provider has already exited. A
  // burn/loop/wedged/assessor_stuck streak that only crosses its threshold
  // at the very last check-in (e.g. a terminal usage envelope only parseable
  // once the stream ends, or a stdout/tick event that lands after exit but
  // before close) must still be recorded as evidence, but it is a post-exit
  // artifact, not a kill that happened while the process was alive -- so it
  // must never latch `this.stopped` (which snapshot()/phase() expose to
  // /api/runs/active and the dashboard). `this.exited` is sticky once set:
  // every later evaluate() call defaults to `latch: false` regardless of
  // which code path or call site invokes it, so no caller needs to
  // separately remember not to latch after exit. Only a verdict latched
  // while the process was alive (before markExited() was ever called) may
  // ever stop a run.
  markExited(now = Date.now()) {
    this.exited = true;
    return this.evaluate(now);
  }

  #stopOrNotify(reason, detail, now, { latch = true } = {}) {
    ...
    if (!latch) {
      if (!this.postExitSignal) this.postExitSignal = { reason, detail, at: now };
      return { action: 'continue', reason: 'post_exit_signal', detail };
    }
  ...
  snapshot(now = Date.now()) {
    ...
    return {
      ...
      unsampledStall: this.unsampledStall ? { ...this.unsampledStall } : null,
      postExitSignal: this.postExitSignal ? { ...this.postExitSignal } : null,
      // Compatibility field (Refs #133): there is no hard cap -- runs are
      // never stopped by elapsed time -- but older callers (continuity
      // integration test, server.js active-run mapping) still read this key
      // and expect it present as null rather than undefined.
