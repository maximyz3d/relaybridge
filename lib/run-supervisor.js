...
class RunSupervisor {
  constructor(options = {}) {
    ...
    this.nextAssessmentAt = this.startedAt + Math.min(this.opts.hardCapMs, this.opts.idleMs);
  ...
  recordOutput(chunk, now = Date.now(), channel = 'stdout') {
    ...
    for (const segment of segments) {
      ...
      // Repeat analysis only looks at substantial lines: spinners, "..." and
      // short separators repeat legitimately in healthy output.
      ...
      // Novelty is tracked for EVERY non-empty line, short ones included. The
      // short-line filter used to skip this update too, so a provider whose
      // progress output is under loopMinChars ("[3/50]", "ok", a short JSONL
      // fragment) never advanced lastNewContentAt: 120 strictly distinct lines
      // still tripped the churn check in evaluate() at exactly noNewContentMs
      // and the receipt blamed a loop that never happened.
  ...
  evaluate(now = Date.now()) {
    ...
    let finalizationCandidate = null;
    if (this.providerUsage) {
      ...
      if (this.opts.finalizationSupported === true
          && !this.finalizationRequested && this.providerUsagePhase === 'incremental') {
        for (const [budgetField, configuredReserve] of Object.entries(this.opts.providerBudgetFinalizationReserve)) {
          ...
          if (observed >= threshold) {
            const request = { budgetField, usageField, observed, limit, reserve, threshold };
    ...
    if (this.opts.hardDeadline !== false && age >= this.opts.hardCapMs) {
    ...
    if (this.opts.adaptive === true) {
      if (this.progress.corroboratedStall(now, this.opts.noNewContentMs)) {
        return this.#stop('assessor_stuck', 'Current progress assessment and sustained repeated failures or repetition support stopping.');
      }
      if (finalizationCandidate) return finalizationCandidate;
      return { action: 'continue', reason: now >= this.nextAssessmentAt ? 'assessment_due' : this.phase(now), detail: '' };
    }
    ...
    if (repeat.count >= this.opts.loopRepeatThreshold) {
      return this.#stop('loop_detected',
        `the same line repeated ${repeat.count} times`);
    }
    ...
    // Finalization is advisory. Every kill guard above retains precedence when
    // the reserve threshold and a hard stop are observed on the same tick.
  ...
  snapshot(now = Date.now()) {
    ...
    return {
      ...
      assessor: { ...this.assessor, verdict: this.progress.assessment },
      ...
      hardCapRemainingMs: this.opts.hardDeadline === false ? null : Math.max(0, this.opts.hardCapMs - (now - this.startedAt)),
...
function resolveSupervisorOptions({ entry = {}, globals = {}, providerBudget, taskTier = null, hardCapMs = null, startedAt } = {}) {
  ...
  // The retired shipped 45-minute default is an assessment interval. A custom
  // operator ceiling or a caller's timeout remains an absolute deadline.
  ...
  if (Number.isFinite(Number(hardCapMs)) && Number(hardCapMs) > 0) {
    merged.hardDeadline = true;
    merged.hardCapMs = Number(hardCapMs);
    // An explicit caller deadline is an absolute ceiling. Keep idle logic
    // reachable by lowering its window, never by extending that deadline to
    // the provider's generous default quiet-start allowance.
    const configuredIdle = Number(merged.idleMs);
    merged.idleMs = Math.min(Number.isFinite(configuredIdle) && configuredIdle > 0
      ? configuredIdle : DEFAULTS.idleMs, merged.hardCapMs);
  }
