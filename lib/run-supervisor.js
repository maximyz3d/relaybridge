...
class RunSupervisor {
  ...
  evaluate(now = Date.now(), { latch = true } = {}) {
  ...
  // N-record-only (Refs #133): the provider has already exited by the time
  // the caller's post-exit settle path runs supervisor.evaluate(). A
  ...
  // once the stream ends) must still be recorded as evidence, but it is a
  // post-exit artifact, not a kill that happened while the process was
  // alive -- so it must never latch `this.stopped` (which snapshot()/phase()
  // expose to /api/runs/active and the dashboard). Only a verdict latched
  // while the process was alive may ever stop a run.
  markExited(now = Date.now()) {
    return this.evaluate(now, { latch: false });
  }
