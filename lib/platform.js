...
function isSlowWslInteropPath(value) {
  ...
  // Keep the lexical WSL mount rule independent of the host running a test or
  // installer preflight. path.resolve('/mnt/c/...') uses a drive path under
  // native Windows, but the supplied value still denotes DrvFs when evaluated
  // for a simulated/remote WSL runtime.
  const portableInput = value.trim().replace(/\\/g, '/');
  if (portableInput === '/mnt' || portableInput.startsWith('/mnt/')) return true;
  let resolved = path.resolve(value);
  // Output files often do not exist yet. Resolve the deepest existing parent
  // so `/home/user/runtime -> /mnt/c/runtime` cannot hide a DrvFs target merely
  // by naming a not-yet-created child.
  let head = resolved;
  let tail = '';
  for (;;) {
    try {
      const real = fs.realpathSync(head);
      resolved = tail ? path.join(real, tail) : real;
      break;
    } catch { /* walk toward an existing ancestor */ }
...
function wslNativeRuntimeStatus(locations = {}, options = {}) {
  ...
  const labels = ['checkout', 'data', 'token', 'config', 'githubRegistry', 'node'];
