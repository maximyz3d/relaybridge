...
async function readBoundedJson(url, { signal, timeoutMs = 4000, maxBytes = 1048576, fetchImpl = fetch } = {}) {
  ...
  let response, reader;
  try {
    response = await fetchImpl(url, { method: 'GET', redirect: 'manual', signal: combined });
    if (!response.ok) {
