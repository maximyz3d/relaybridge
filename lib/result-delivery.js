...
function sanitizeResult(payload = {}, failed = false, task = {}) {
  ...
  const partial = Boolean(failed || payload.partial_result === true || payload.dropped_out === true || payload.ok === false
...
function resultProjection(task, receiptStoreId, expectedId = task?.id) {
  ...
  const metadata = available ? {
    ...
    originalBytes: stored.originalBytes, complete: task.status === 'done' && stored.complete === true,
    partial: task.status === 'failed' || stored.partial === true,
    modelInvocation: stored.modelInvocation, providerCompleted: stored.providerCompleted,
