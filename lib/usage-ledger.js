...
function createUsageLedger(opts = {}) {
  ...
  function observeVendorQuota(observation) {
    ...
    const row = {
      ...
      quotaSeat: quotaSeatFor(observation.provider),
  ...
  function activeVendorQuota(seat, model = null) {
    ...
    const wantedQuotaSeat = quotaSeatFor(seat);
    let latest = null;
  ...
  function operatorQuotaObservations({ includeExpired = false } = {}) {
    if (!fs.existsSync(operatorQuotaPath)) return {};
    const latest = new Map();
    for (const line of fs.readFileSync(operatorQuotaPath, 'utf8').split('\n')) {
      if (!line.trim()) continue;
      try {
        const row = JSON.parse(line);
  ...
  function record(entry) {
    ...
    const row = {
      ...
      quotaSeat: quotaSeatFor(entry.seat || 'unknown'),
  ...
  function burnRate(seat, windowMs = 3600000) {
    const quotaSeat = quotaSeatFor(seat);
  ...
  function gauge(seat, seatConfig = {}, windowMs = DAY_MS) {
    ...
    const used = summarize(rows({ sinceMs: windowMs }).filter((row) =>
      (row.quotaSeat || quotaSeatFor(row.seat)) === quotaSeat));
    const burn = burnRate(seat);
    const budget = budgets[quotaSeat] || budgets[seat] || {};
