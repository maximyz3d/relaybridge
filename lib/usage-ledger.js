...
const { normalizeQualitativeQuotaExhaustion, activeQualitativeQuotaExhaustion } = require('./vendor-quota');
...
function createUsageLedger(opts = {}) {
  ...
  function observeVendorQuota(observation) {
    ...
    if (observation.kind !== undefined) {
      const normalized = normalizeQualitativeQuotaExhaustion(observation);
      if (!normalized) return null;
      const row = { ...normalized, seat:normalized.provider,
        quotaSeat:typeof observation.quotaSeat === 'string' && QUOTA_SEAT_RE.test(observation.quotaSeat)
          ? observation.quotaSeat : quotaSeatFor(normalized.provider), recordedAt:new Date(now()).toISOString() };
      try { fs.appendFileSync(vendorQuotaPath, JSON.stringify(row) + '\n', 'utf8'); }
      catch { log('[RelayBridge] qualitative vendor quota observation write failed'); return null; }
      return row;
    }
  ...
  function activeVendorQuota(seat, model = null, quotaSeatOverride = null) {
    ...
    let latest = null, qualitative = null;
    for (const row of parsedRows(vendorQuotaPath)) {
      try {
        ...
        if (row.kind !== undefined) {
          const valid = activeQualitativeQuotaExhaustion(row, current);
          if (valid && (!qualitative || Date.parse(valid.reset.expiresAt) > Date.parse(qualitative.reset.expiresAt))) qualitative = { ...row, ...valid };
          continue;
        }
  ...
  function gauge(seat, seatConfig = {}, windowMs = DAY_MS) {
    ...
    const nativeUsage = typeof opts.nativeHeadroom === 'function'
      ? opts.nativeHeadroom(quotaSeat, { model: seatConfig.model || null }) : null;
    const operatorQuota = activeOperatorQuota(quotaSeat);
    if (vendorQuota) {
      basis = 'vendor_observed';
      capacity = vendorQuota.limit;
      remaining = vendorQuota.kind === 'quota_exhausted' ? null : Math.max(0, vendorQuota.limit - vendorQuota.actual);
      percentRemaining = vendorQuota.kind === 'quota_exhausted' ? null : Math.max(0, Math.round((remaining / capacity) * 100));
      hoursToEmpty = null;
    } else if (nativeUsage?.freshness === 'fresh') {
      basis = 'native_observed';
      capacity = null;
      remaining = null;
      percentRemaining = nativeUsage.percentRemaining;
      hoursToEmpty = nativeUsage.percentPerHour ? percentRemaining / nativeUsage.percentPerHour : null;
      ...
      hoursToEmpty = null;
    } else if (costClass === 'subscription') {
      // Local token totals (including cached context) do not measure a
      // provider's subscription allowance. Keep budget estimates separate
      // so consumers cannot mistake an exhausted estimate for an empty seat.
      if (basis === 'unmetered') basis = 'unknown';
      capacity = null;
      remaining = null;
    ...
    return {
      ...
      nativeUsage,
      reserveProtected: nativeUsage?.protected === true,
      configuredEstimate: (vendorQuota || operatorQuota || costClass === 'subscription') && configuredEstimate.basis !== 'unmetered' ? configuredEstimate : null,
      used: {
      ...
      // as fact.
      note: basis === 'native_observed' ? 'Native provider subscription windows; freshness and applicable quota bucket are included.' : basis === 'vendor_observed'
        ? vendorQuota?.kind === 'quota_exhausted'
          ? 'vendor-observed quota exhaustion; subscription allowance unknown; blocked until the reported reset or bounded conservative expiry'
        ...
        : basis === 'configured'
        ? 'subscription remaining capacity unknown; configured estimate is not a vendor-published quota and must not gate or deprioritize this seat'
        : basis === 'unknown' ? 'subscription remaining capacity unknown; no current vendor or operator observation'
