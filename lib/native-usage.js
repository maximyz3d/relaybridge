...
function readClaudeNativeUsage({ env = process.env, defaultHome = os.homedir(), quotaSeat, now = Date.now(), fsImpl = fs } = {}) {
  const unavailable = { identity: null, observation: null, reason: 'native_profile_unavailable' };
  ...
  try {
    ...
    if (!before.isFile() || !same(initial, before)) return unavailable;
    const buffer = Buffer.alloc(1024 * 1024 + 1);
    let bytes = 0, got;
    while (bytes < buffer.length && (got = fsImpl.readSync(fd, buffer, bytes, buffer.length - bytes, null))) bytes += got;
    if (bytes > 1024 * 1024 || BigInt(bytes) !== before.size
      || !same(before, fsImpl.fstatSync(fd, { bigint: true }))
      || !same(before, fsImpl.lstatSync(file, { bigint: true }))) return unavailable;
    ...
    return { identity, observation, reason: observation ? null : 'native_cache_unavailable' };
