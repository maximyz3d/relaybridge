...
function disconnectFailureClass({ client = null, deadlineAt = null, now = Date.now() } = {}) {
  // deadlineAt === null means the run has no deadline (uncapped): Number(null)
  // is 0, which is finite, so without this guard every no-deadline MCP
  // disconnect would be misclassified as a deadline-triggered one.
  if (deadlineAt === null || deadlineAt === undefined) return 'client_cancelled';
