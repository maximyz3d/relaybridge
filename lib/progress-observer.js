...
class ProgressObserver {
  constructor({ parser = 'text', startedAt = Date.now(), runId = null, attemptId = null } = {}) {
    Object.assign(this, { parser, startedAt, runId, attemptId, lastActivityAt: startedAt,
      ...
      lastToolActivityAt: startedAt, materialGeneration: 0, claudeTools: new Map() });
    ...
    if (this.parser === 'claude_json') {
      if (value.type === 'assistant' && Array.isArray(value.message?.content)) {
        ...
        for (const b of value.message.content.filter((b) => b?.type === 'tool_use')) {
          ...
          const key = hash(b.id);
          if (this.claudeTools.has(key)) continue;
          if (this.claudeTools.size >= 4096) this.claudeTools.delete(this.claudeTools.keys().next().value);
          this.claudeTools.set(key, false);
        ...
        for (const b of value.message.content) {
          ...
          const key = hash(b.tool_use_id);
          if (this.claudeTools.get(key) !== false) continue;
          this.claudeTools.set(key, true);
          const failure = b.is_error === true;
          this.counts.toolsCompleted++;
          if (failure) this.counts.toolsFailed++;
          this.lastToolActivityAt = now; this.materialGeneration++;
          // Never retain tool arguments, result contents or unhashed identities.
          this.add('tool', failure ? 'Tool failed' : 'Tool completed', `${key}:completed`, now,
            { useful: !failure, failure });
        }
