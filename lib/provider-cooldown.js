...
function createCooldownStore(opts = {}) {
   ...
   */
  function noteFailure(seat, kind, { retryAfterSec = null } = {}) {
    ...
    e.until = Math.max(e.until, t + ms);
    e.reason = kind;
    e.source = source;
    persist();
    log(`[RelayBridge] ${seat} cooling for ${Math.round(ms / 1000)}s (${kind}, ${source})`);
    return { seat, until: e.until, reason: kind, source, offences: e.offences };
  }
