...
// * Explicit provider selection still obeys cooldown authority. Authentication
//   alone cannot authorize another paid call into an exhausted subscription.
...
const { own, validQuotaSeat } = require('./quota-seat');
...
function createCooldownStore(opts = {}) {
  ...
  function assertSeat(seat) {
    if (!validQuotaSeat(seat)) {
      throw Object.assign(new Error('invalid quota seat'), { code: 'invalid_quota_seat' });
    }
  }

  let unknown = false;
  let invalidSeats = new Set();
  let invalidRows = Object.create(null);
  let existed = false;

  function normalizeState(raw) {
    const result = Object.create(null);
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) throw new Error('invalid cooldown authority');
    const bad = new Set(), badRows = Object.create(null);
    for (const [seat, row] of Object.entries(raw)) {
      if (!validQuotaSeat(seat) || !row || typeof row !== 'object' || Array.isArray(row) ||
          !['until', 'lastOffenceAt', 'offences'].every((key) => own(row, key) &&
            typeof row[key] === 'number' && Number.isFinite(row[key]) && row[key] >= 0) ||
          !Number.isSafeInteger(row.offences)) {
        if (!validQuotaSeat(seat)) throw new Error('invalid cooldown seat');
        bad.add(seat);
        badRows[seat] = row;
        continue;
      }
      result[seat] = {
        until: row.until, lastOffenceAt: row.lastOffenceAt, offences: row.offences,
        reason: typeof row.reason === 'string' ? row.reason.slice(0, 80) : null,
        source: typeof row.source === 'string' ? row.source.slice(0, 80) : null,
        scope: row.scope === 'model' ? 'model' : 'account',
        sourceObservationId: typeof row.sourceObservationId === 'string' && /^[a-f0-9]{24}$/.test(row.sourceObservationId) ? row.sourceObservationId : null,
        sourceReceiptId: /^rcpt_[a-z0-9_]{1,100}$/.test(row.sourceReceiptId || '') ? row.sourceReceiptId : null,
      };
    }
    invalidSeats = bad;
    invalidRows = badRows;
  ...
  try {
    ...
    unknown = true;
    log('[RelayBridge] cooldown authority unreadable; admission is blocked pending repair');
  }
  ...
  function syncFromDisk() {
    ...
    try { stat = fs.statSync(file); } catch (error) {
      if (existed || error.code !== 'ENOENT') unknown = true;
      return;
    }
    if (stat.mtimeMs === seen.mtimeMs && stat.size === seen.size) return;
    let disk = null;
    try {
      disk = normalizeState(JSON.parse(fs.readFileSync(file, 'utf8')));
      if (unknown) state = Object.create(null); // explicit valid repair replaces unavailable authority
      unknown = false;
      existed = true;
    } catch {
      unknown = true;
    ...
    for (const [seat, e] of Object.entries(disk)) {
      const mine = own(state, seat) ? state[seat] : null;
      if (mine && mine.lastOffenceAt > e.lastOffenceAt) continue;
      state[seat] = e;
    }
  }

  function persist() {
    // A successful run or a new failure cannot repair unrelated missing evidence.
    if (unknown) return false;
    try {
      const tmp = `${file}.${process.pid}.${require('crypto').randomBytes(6).toString('hex')}.tmp`;
      const fd = fs.openSync(tmp, 'wx', 0o600);
      try { fs.writeFileSync(fd, JSON.stringify({ ...state, ...invalidRows }, null, 2), 'utf8'); fs.fsyncSync(fd); } finally { fs.closeSync(fd); }
      ...
      unknown = true;
      log('[RelayBridge] cooldown persistence failed; authority unavailable');
      return false;
    }
  }
  ...
  function entry(seat) {
    assertSeat(seat);
    return own(state, seat) ? state[seat] : (state[seat] = { offences: 0, lastOffenceAt: 0, until: 0, reason: null, source: null });
  }
   ...
   */
  function noteFailure(seat, kind, { retryAfterSec = null, scope = 'account', sourceReceiptId = null } = {}) {
    assertSeat(seat);
    ...
    const authorityChanged = proposedUntil >= e.until;
    if (authorityChanged) {
      e.until = proposedUntil;
      e.reason = kind;
      e.source = source;
      e.scope = scope === 'model' ? 'model' : 'account';
      e.sourceObservationId = require('crypto').randomBytes(12).toString('hex');
      e.sourceReceiptId = /^rcpt_[a-z0-9_]{1,100}$/.test(sourceReceiptId || '') ? sourceReceiptId : null;
    }
    persist();
    log(`[RelayBridge] ${seat} cooling for ${Math.ceil((e.until - t) / 1000)}s (${e.reason}, ${e.source})`);
    return { seat, until: e.until, reason: e.reason, source: e.source, offences: e.offences, scope: e.scope, sourceReceiptId: e.sourceReceiptId || null, sourceObservationId: e.sourceObservationId || null, authorityChanged };
  }
  ...
  function noteSuccess(seat, { startedAt = null } = {}) {
    assertSeat(seat);
    syncFromDisk();
    if (unknown || invalidSeats.has(seat)) return null;
    const e = state[seat];
    if (!e || !e.until) return null;
    if (Number.isFinite(startedAt) && startedAt <= e.lastOffenceAt) return null;
    e.until = 0;
    e.reason = null;
    e.source = null;
    persist();
  ...
  // Attach only the exact successfully persisted execution receipt. Concurrent
  // newer observations win; a failed receipt append cannot mint provenance.
  function attachReceipt(observation, receiptId) {
    if (!observation || !/^rcpt_[a-z0-9_]{1,100}$/.test(receiptId || '')) return false;
    syncFromDisk();
    const e = state[observation.seat];
    if (unknown || invalidSeats.has(observation.seat) || !e || observation.authorityChanged !== true
      || !observation.sourceObservationId || e.sourceObservationId !== observation.sourceObservationId) return false;
    e.sourceReceiptId = receiptId;
    return persist();
  }
  ...
  function status(seat) {
    assertSeat(seat);
    syncFromDisk();
    if (unknown || invalidSeats.has(seat)) return { seat, cooling: true, authority: 'unknown',
      reason: 'cooldown_authority_unavailable', until: null, remainingMs: null, remainingSec: null, scope: 'account' };
    const e = state[seat];
    const t = now();
    ...
    return {
      ...
      reason: e.reason, source: e.source, offences: e.offences, sourceReceiptId: e.sourceReceiptId || null,
  ...
  function all() {
    ...
    for (const seat of new Set([...Object.keys(state), ...invalidSeats])) out[seat] = status(seat);
    return out;
  }
   ...
   * missing binary or a bad config. Explicit selections obey the same authority.
   */
  function filterCandidates(candidates = []) {
    ...
    for (const c of candidates) {
      ...
      const s = status(seat);
      if (s.cooling) skipped.push({ ...(typeof c === 'string' ? { seat } : c), cooldown: s });
  ...
  return { noteFailure, noteSuccess, attachReceipt, status, all, cooling, filterCandidates, _state: () => state };
}
