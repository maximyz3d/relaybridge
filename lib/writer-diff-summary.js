...
function summarizeWriterWorkspaceDiff(before, after) {
  ...
  const unverified = [...paths].filter((file) => before.entries.get(file) === after.entries.get(file)
  ...
  // Hash public redacted metadata only, including public ordering. Raw secret
  // filenames must not be recoverable by recomputing a dictionary candidate.
  const canonical = changed.map((file) => [isSecretPath(file) ? '[redacted-sensitive-path]' : file,
    before.entries.get(file) || null, after.entries.get(file) || null])
  ...
  return {
    ...
    changedFileCount: unverified.length ? null : changed.length,
    changedFileCountLowerBound: changed.length,
    unverifiedFileCount: unverified.length,
    changeCountComplete: unverified.length === 0,
    files,
