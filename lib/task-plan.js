...
const { resolveAttemptTiming, renderCliDeadline } = require('./cli-deadline');
const { resolveProviderControls } = require('./execution-contract');
const { hardEligibility, loadAptitudes } = require('./routing-eligibility');
...
// router (tier + ranked providers). The shared execution resolver owns every
// provider model and effort interpretation.
function buildTaskPlan({
  ...
  requestedKind = null,
  requestedProviderBudget = null,
  requestedModel = null,
  requestedModelTier = null,
  requestedExecution = null,
  requestedMaxEffortOverride = false,
  ...
  const aptitudes = loadAptitudes();

  const candidates = requestedKind
    ? selected.filter((pick) => pick.kind === requestedKind).concat(
      selected.some((pick) => pick.kind === requestedKind) ? []
        : [(route.candidates || []).find((pick) => pick.kind === requestedKind)
          || { kind: requestedKind, reason: 'explicitly requested' }])
    : selected;

  const vendorQuotaSkipped = Array.isArray(route.fleetState?.vendorQuotaSkipped)
    ? route.fleetState.vendorQuotaSkipped : [];

  const plans = candidates.map((pick) => {
    const entry = config[pick.kind] || {};
    const cost = costClassFor(entry, pick.kind);
    let controls = null, validation = null;
    if (cost !== 'none') {
      try {
        controls = resolveProviderControls({ kind: pick.kind, entry, registry,
          slot: dangerous ? entry.oneshot_dangerous : entry.oneshot_safe,
          taskTier: tier, modelTier: requestedModelTier, model: requestedModel,
          effort: requestedEffort, execution: requestedExecution, maxEffortOverride: requestedMaxEffortOverride, phase: 'plan', dangerous });
      } catch (error) { validation = error.validation || { code: 'invalid_provider_controls', field: 'provider', reason: 'Provider controls cannot be planned.', retryable: false }; }
    }
    const hard = hardEligibility({ kind: pick.kind,
      capabilities: entry.oneshot_capabilities?.[dangerous ? 'dangerous' : 'safe'],
      aptitude: aptitudes[pick.kind] || {}, classification: route.classification || { tier }, modelTier: requestedModelTier });
    const ineligibilityReasons = [...hard.ineligibilityReasons, ...(pick.ineligibilityReasons || [])];
    if (!hard.eligible && !validation) validation = { code: 'ineligible_provider', field: 'kind',
      reason: hard.ineligibilityReasons.join('; '), retryable: false };

    const supervisorOptions = resolveAttemptTiming({
      entry,
      globals: config._supervisor || {},
      providerBudget: requestedProviderBudget,
      taskTier: tier,
      timeoutMs: requestedTimeoutMs,
    });
    let cliDeadline = null;
    if (controls) {
      try { cliDeadline = renderCliDeadline({ entry, slot: controls.slot, supervisorOptions }); }
      catch (error) { validation = error.validation; }
    }

    const vendorQuotaBlock = vendorQuotaSkipped.find((item) => item.kind === pick.kind) || null;
    return {
      company: entry.company || entry.label || pick.kind,
      kind: pick.kind,
      label: entry.label || pick.kind,
      model: controls?.execution.model ?? null,
      modelTier: controls?.execution.resolvedModelTier ?? null,
      effort: controls?.execution.targetEffort ?? effort,
      appliedEffort: controls?.execution.appliedEffort ?? null,
      effortMethod: controls?.execution.effortMethod ?? (cost === 'none' ? 'not_applicable' : 'unsupported'),
      effortSupported: !validation,
      effortFallbackReason: controls?.execution.effortFallbackReason ?? null,
      args: cliDeadline?.slot.slice(1) ?? controls?.slot.slice(1) ?? [],
      effectiveTimeoutMs: supervisorOptions.hardDeadline === false ? null : supervisorOptions.hardCapMs,
      requestedTimeoutMs: Number.isFinite(Number(requestedTimeoutMs)) && Number(requestedTimeoutMs) > 0 ? Number(requestedTimeoutMs) : null,
      cliDeadline: cliDeadline?.deadline ?? null,
      execution: controls?.execution ?? null,
      validation,
      eligible: hard.eligible && pick.eligible !== false && !validation,
      ineligibilityReasons: [...new Set(ineligibilityReasons)],
      costClass: cost,
      costNote: COST_CLASS[cost].label,
      ready: !validation && pick.eligible !== false && !vendorQuotaBlock && pick.ready !== false && pick.readiness?.ready !== false,
      blocked: !!validation || pick.eligible === false || !!vendorQuotaBlock || pick.ready === false || pick.readiness?.ready === false,
      filesystem: pick.readiness?.safeFilesystem || null,
      vendorQuotaBlock,
      reason: vendorQuotaBlock?.reason || pick.reason || null,
      providerBudget: supervisorOptions.providerBudget,
    };
  });
  ...
  const ordered = plans.filter((pick) => pick.ready && !pick.blocked).sort((a, b) => COST_CLASS[a.costClass].rank - COST_CLASS[b.costClass].rank);
  const primary = plans[0] || null;

  const guidance = [];
  if (primary?.validation) guidance.push(primary.validation.reason);
  if (!primary) {
    const filesystemSkipped = route.fleetState?.filesystemSkipped || [];
    if (vendorQuotaSkipped.length) {
      guidance.push(`No quota-eligible provider is available; vendor quota is exhausted until reset: ${vendorQuotaSkipped.map((item) => `${item.kind} (${item.retry?.allowedAfter || 'reset time unavailable'})`).join(', ')}. Use an alternate quota seat until then.`);
    } else if (filesystemSkipped.length) {
      guidance.push(`No safe-eligible provider is available; blocked before invocation: ${filesystemSkipped.map((item) => `${item.kind} (${item.policy})`).join(', ')}.`);
    } else {
      // Returning a null plan with no explanation leaves a caller with nothing
      // to act on. Say why there is no plan and what would fix it.
    ...
    const blocked = (route.candidates || []).filter((pick) => pick.eligible === false).slice(0, 12);
    if (blocked.length) guidance.push('Eligibility gates: ' + blocked.map((pick) => `${pick.kind}: ${(pick.ineligibilityReasons || []).join('; ')}`).join(' | '));
  }
  if (primary?.blocked && primary.filesystem?.eligible === false) {
    guidance.push(`${primary.label} was explicitly requested but is blocked before invocation: ${primary.filesystem.blockedReason}. Use a safe-eligible provider or separately authorize dangerous writer planning.`);
  }
  if (primary?.vendorQuotaBlock) {
    guidance.push(`${primary.label} was explicitly requested but its vendor quota is exhausted. Do not retry before ${primary.vendorQuotaBlock.retry.allowedAfter}; use an alternate quota seat until reset.`);
  }
  ...
  if (primary && primary.effortMethod === 'model_choice') {
    guidance.push(`${primary.label} uses the ${primary.model || 'account default'} model tier; consult appliedEffort and the exact planned args for runtime effort controls.`);
  }
  if (tier === 'critical') {
