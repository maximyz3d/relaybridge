...
function sanitizeCheckpointSecrets(value, maxInputBytes = DEFAULT_MAX_REDACTION_INPUT_BYTES) {
  ...
  // When the bounded input begins in the middle of a key block, its BEGIN
  // marker is outside the retained tail. An END marker makes that prefix
  // security-sensitive, so discard it through the marker rather than emit key
  // material whose opening delimiter was truncated away.
