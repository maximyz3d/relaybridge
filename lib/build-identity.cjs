...
const BUILD_INFO_VERIFY_ATTEMPTS = 4;
...
function writeBuildInfoAtomic(root, info, options = {}) {
  ...
  try {
    ...
    handle = null;
    // Windows readers (including another source preparation) can briefly hold
    // a handle that denies replacement. Keep the old manifest intact and retry
    // the same atomic rename; never unlink the destination to make room.
    const platform = options.platform || process.platform;
    const pause = options.renameRetryPause || ((ms) => {
...
function prepareBuildInfo(root, options = {}) {
  ...
  // A concurrent atomic publication can correctly fail one stable-file read.
  // Retry only that invalid result, fully revalidating source and manifest each
  // time. The bounded retry never relaxes a guard or accepts another buildId.
  for (let attempt = 0; attempt < BUILD_INFO_VERIFY_ATTEMPTS; attempt += 1) {
    ...
    if (identity.ready || identity.reason !== 'build_info_invalid') break;
  }
