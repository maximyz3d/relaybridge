...
function createSubscriptionUsage({ dataDir, now = Date.now, ttlMs = TTL_MS } = {}) {
  ...
  function headroom(quotaSeat, { model = null, bucket: requestedBucket = null, accountFingerprint = null } = {}) {
    ...
    // S5: a window whose own resetsAt has already passed is not "still low" -- its
    // capacity has renewed even though the percentRemaining evidence for the new
    // window has not arrived yet. Such a window must not keep protecting the seat by
    // itself, so a reserve-filtered account-selection or admission check does not
    // reject as quota_reserve once the binding window's reset time is behind us.
    const low = windows.some((w) => typeof w.percentRemaining === 'number' && w.percentRemaining <= settings.reservePercent
      && !(Number.isSafeInteger(w.resetsAt) && now() >= w.resetsAt));
  ...
  return { observe: observation => observe(observation),
    ...
    bindIdentity, headroom, fingerprint: (seat) => seats[seat]?.accountFingerprint || null, list: () => Object.keys(seats).map((key) => headroom(key)),
