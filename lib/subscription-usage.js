...
function createSubscriptionUsage({ dataDir, now = Date.now, ttlMs = TTL_MS } = {}) {
  ...
  function sameNativeWindow(previous, window) {
    const raw = window.nativeResetMs ?? previous.nativeResetMs;
    if (!Number.isSafeInteger(raw)) return previous.resetsAt === window.resetsAt;
    const rawNs = window.nativeResetNs ?? previous.nativeResetNs ?? String(BigInt(raw) * 1000000n);
    const boundary = Number((BigInt(rawNs) + 999999999n) / 1000000000n) * 1000;
    if (previous.nativeResetMs != null && window.nativeResetMs != null
      && (previous.nativeResetMs !== window.nativeResetMs
        || (previous.nativeResetNs ?? String(BigInt(previous.nativeResetMs) * 1000000n)) !== rawNs)) return false;
    return [previous, window].every((w) => w.nativeResetMs != null
      ? w.nativeResetMs === raw : w.resetsAt === boundary);
  }
  function observe(observation) {
    ...
    if (native && (!/^[a-f0-9]{64}$/.test(observation.accountFingerprint || '')
      ...
      || !['five_hour', 'seven_day'].every((id) => observation.buckets[0].windows.some((w) => w.id === id && !w.invalid
        ...
        && w.nativeResetMs === w.resetsAt && w.resetsAt > observation.observedAt)))) return false;
    ...
    for (const bucket of observation.buckets) for (const w of bucket.windows) {
      const prior = old?.buckets[bucket.id]?.windows?.[w.id];
      if (!prior || w.invalid || !(native || prior.nativeResetMs != null)) continue;
      const same = sameNativeWindow(prior, w);
      if (same && w.percentRemaining > prior.percentRemaining) return rejectCapacity();
      const boundary = prior.resetBoundaryMs ?? prior.resetsAt;
    ...
    for (const bucket of observation.buckets) {
      ...
      for (const window of bucket.windows) {
        ...
        const sameReset = previous && (native || previous.nativeResetMs != null)
          ? sameNativeWindow(previous, window) : previous?.resetsAt === window.resetsAt;
        if (sameReset && (native || previous?.nativeResetMs != null)) {
          Object.assign(value, { nativeResetMs: window.nativeResetMs ?? previous.nativeResetMs,
            nativeResetNs: window.nativeResetNs ?? previous.nativeResetNs,
            nativeUsedPercent: window.nativeUsedPercent ?? previous.nativeUsedPercent,
            nativeResetIso: window.nativeResetIso ?? previous.nativeResetIso,
            resetBoundaryMs: window.resetBoundaryMs ?? previous.resetBoundaryMs,
        ...
        if (!window.invalid && previous && (!previous.invalid || previous.nativeResetMs != null) && sameReset) {
        ...
        if (window.invalid) {
          ...
          if (previous?.nativeResetMs != null) for (const field of ['nativeResetMs', 'nativeResetNs', 'nativeResetIso',
            'nativeUsedPercent', 'resetBoundaryMs', 'observedAt', 'percentPerHour', 'rateAnchorPercentRemaining', 'rateAnchorObservedAt']) {
  ...
  function headroom(quotaSeat, { model = null, bucket: requestedBucket = null, accountFingerprint = null } = {}) {
    ...
    if (windows.some((w) => w.invalid)) return unknown('native_evidence_invalid', {
