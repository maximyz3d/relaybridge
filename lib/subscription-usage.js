...
function createSubscriptionUsage({ dataDir, now = Date.now, ttlMs = TTL_MS } = {}) {
  ...
  function nativeResetIdentity(window) {
    ...
    if (!Number.isSafeInteger(boundary)
      || window.resetBoundaryMs != null && window.resetBoundaryMs !== boundary) return null;
  ...
  function sameNativeWindow(previous, window) {
    const before = previous.nativeResetMs != null ? nativeResetIdentity(previous) : null;
    const after = window.nativeResetMs != null ? nativeResetIdentity(window) : null;
    if (previous.nativeResetMs != null && !before || window.nativeResetMs != null && !after) return false;
    if (!before && !after) return previous.resetsAt === window.resetsAt;
    // Native refreshes jitter within the same exact ceil-second window. A
    // fixed key is transitive; it cannot accumulate a sliding time tolerance.
    const boundary = (before || after).boundary;
    return (before ? before.boundary === boundary : previous.resetsAt === boundary)
      && (after ? after.boundary === boundary : window.resetsAt === boundary);
  }
  function observe(observation) {
    ...
    for (const bucket of observation.buckets) for (const w of bucket.windows) {
      ...
      const priorIdentity = prior.nativeResetMs != null ? nativeResetIdentity(prior) : null;
      if (prior.nativeResetMs != null && !priorIdentity) return rejectCapacity();
    ...
    for (const bucket of observation.buckets) {
      ...
      for (const window of bucket.windows) {
        ...
        if (identity) value.resetBoundaryMs = identity.boundary;
        ...
        if (sameReset && (native || previous?.nativeResetMs != null)) {
          ...
          Object.assign(value, { nativeResetMs: earliest.nativeResetMs,
            ...
            resetBoundaryMs: (before || after).boundary,
