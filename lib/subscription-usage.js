...
function createSubscriptionUsage({ dataDir, now = Date.now, ttlMs = TTL_MS } = {}) {
  ...
  function hasNativeResetAuthority(window) {
    return window.source === 'claude_native_cache_v1'
      || ['nativeResetMs', 'nativeResetNs', 'nativeResetIso', 'nativeUsedPercent',
        'nativeResetAnchor', 'resetBoundaryMs', 'nativeNoActiveWindow'].some(field => Object.hasOwn(window, field));
  }
  function nativeNoActiveWindow(window) {
    return window?.id === 'five_hour' && window.nativeNoActiveWindow === true
      && window.nativeUsedPercent === 0 && window.percentRemaining === 100
      && window.resetsAt === null && window.nativeResetMs === undefined
      && window.nativeResetNs === undefined && window.nativeResetIso === undefined
      && window.nativeResetAnchor === undefined && window.resetBoundaryMs === undefined;
  }
  ...
  function sameNativeWindow(previous, window) {
    if (nativeNoActiveWindow(previous) || nativeNoActiveWindow(window))
      return nativeNoActiveWindow(previous) && nativeNoActiveWindow(window);
  ...
  function observe(observation, nativeCounterRefresh = false) {
    ...
    if (native && (!/^[a-f0-9]{64}$/.test(observation.accountFingerprint || '')
      ...
      || !['five_hour', 'seven_day'].every((id) => observation.buckets[0].windows.some((w) => {
        if (w.id !== id || w.invalid) return false;
        if (nativeNoActiveWindow(w)) return true;
        return typeof w.percentRemaining === 'number' && Number.isFinite(w.percentRemaining)
    ...
    if (observation.buckets.some(b => b.windows.some(w => Object.hasOwn(w, 'nativeResetAnchor')
      || Object.hasOwn(w, 'nativeCounterRefresh')))) return rejectCapacity();
    ...
    // Only the verified native-profile adapter may refresh counters without
    // admitting a changed reset identity. Both windows must retain valid native
    // authority, future conservative expiry and non-increasing capacity.
    const canRefreshCounters = nativeCounterRefresh && native && boundClaude
      && old?.provider === 'claude' && old.defaultBucket === 'account'
      && old.accountFingerprint === observation.accountFingerprint
      && Object.keys(old.buckets?.account?.windows || {}).length === 2
      && observation.buckets[0].windows.every(w => {
        const prior = old.buckets.account.windows[w.id];
        const duration = { five_hour: 18000000, seven_day: 604800000 }[w.id];
        return prior && !prior.invalid && nativeResetAnchor(prior)
          && prior.accountFingerprint === observation.accountFingerprint
          && prior.windowDurationMs === duration && w.windowDurationMs === duration
          && Number.isFinite(prior.percentRemaining) && prior.percentRemaining >= 0 && prior.percentRemaining <= 100
          && w.percentRemaining <= prior.percentRemaining
          && Number.isSafeInteger(prior.observedAt) && prior.observedAt < observation.observedAt
          // The rate estimator needs at least one second for a decrease. Reject
          // the entire refresh instead of clearing protection on either sibling.
          && (w.percentRemaining === prior.percentRemaining || observation.observedAt - prior.observedAt >= 1000)
          && observation.observedAt < prior.resetsAt && now() < prior.resetsAt
          && w.resetsAt >= prior.resetsAt;
      });
    const counterOnlyWindows = new Set();
    for (const bucket of observation.buckets) for (const w of bucket.windows) {
      const prior = old?.buckets[bucket.id]?.windows?.[w.id];
      if (prior && hasNativeResetAuthority(prior) && !nativeResetAnchor(prior)
        && !nativeNoActiveWindow(prior)) return rejectCapacity();
      if (!prior || w.invalid || !(native || hasNativeResetAuthority(prior))) continue;
      const same = sameNativeWindow(prior, w);
      if (same && (w.percentRemaining > prior.percentRemaining
        || prior.resetsAt !== null && observation.observedAt >= prior.resetsAt)) return rejectCapacity();
      ...
      if (!same && boundary !== null && (observation.observedAt < boundary
        || w.resetsAt !== null && w.resetsAt <= boundary)) {
        if (!canRefreshCounters) return rejectCapacity();
        counterOnlyWindows.add(w.id);
      }
      ...
      if (!same && priorIdentity && w.nativeResetMs == null && !nativeNoActiveWindow(w)) {
    ...
    for (const bucket of observation.buckets) {
      ...
      for (const window of bucket.windows) {
        ...
        const counterOnly = counterOnlyWindows.has(window.id);
        if (counterOnly) {
          for (const field of ['nativeResetMs', 'nativeResetNs', 'nativeResetIso',
            'nativeResetAnchor', 'resetBoundaryMs', 'resetsAt']) {
            if (Object.hasOwn(previous, field)) value[field] = structuredClone(previous[field]);
            else delete value[field];
          }
          value.nativeCounterRefresh = { version: 1, resetIdentityAccepted: false,
            observedAt: observation.observedAt, evidenceHash: observation.evidenceHash,
            retainedFromEvidenceHash: old.evidenceHash,
            claimedReset: { iso: window.nativeResetIso, ns: window.nativeResetNs, ms: window.nativeResetMs } };
        }
        if (sameReset && !nativeNoActiveWindow(window) && (native || previous && hasNativeResetAuthority(previous))) {
        ...
        if (!window.invalid && previous && (!previous.invalid || hasNativeResetAuthority(previous)) && (sameReset || counterOnly)) {
        ...
        if (window.invalid) {
          ...
          if (previous && hasNativeResetAuthority(previous)) for (const field of ['nativeResetMs', 'nativeResetNs', 'nativeResetIso',
            'nativeUsedPercent', 'nativeNoActiveWindow', 'nativeResetAnchor', 'resetBoundaryMs',
            'observedAt', 'percentPerHour', 'rateAnchorPercentRemaining', 'rateAnchorObservedAt']) {
  ...
  // S5: whether a window's own anchored rollover has occurred, i.e. the reset boundary
  // that survives invalid/stale evidence (see nativeResetAnchor), not a raw resetsAt that
  // could be forged or moved by an untrusted observation. Windows carrying native reset
  // authority but an invalid/un-verifiable anchor must not be treated as rolled over.
  function anchoredRolloverOccurred(window) {
    if (!window) return false;
    if (hasNativeResetAuthority(window)) {
      const anchor = nativeResetAnchor(window);
      return !!anchor && now() >= anchor.boundary;
    }
    return Number.isSafeInteger(window.resetsAt) && now() >= window.resetsAt;
  }
  function headroom(quotaSeat, { model = null, bucket: requestedBucket = null, accountFingerprint = null } = {}) {
    ...
    const unknown = (reason, extra = {}) => ({ quotaSeat, percentRemaining: null, freshness: seat ? 'stale' : 'unknown',
      source: seat?.source || null, reason, protected: false, vendorBlocked: false, ...extra });
    ...
    if (!bucket) return unknown('quota_bucket_applicability_unknown', { protected: blocked, vendorBlocked: blocked });
    ...
    if (seat.identityNeedsFreshEvidence) return unknown('native_account_capacity_unbound', { protected: protectedState, vendorBlocked: blocked });
    if (accountFingerprint && (seat.accountFingerprint !== accountFingerprint
      ...
      return unknown('native_account_capacity_unbound', { protected: protectedState, vendorBlocked: blocked });
    }
    ...
    if (windows.some((w) => w.invalid || hasNativeResetAuthority(w)
      && !nativeResetAnchor(w) && !nativeNoActiveWindow(w))) return unknown('native_evidence_invalid', {
      bucket: bucketId, windows, protected: protectedState, vendorBlocked: blocked, evidenceHash: seat.evidenceHash,
    ...
    return { quotaSeat, bucket: bucketId, model: bucket.model, percentRemaining, percentPerHour,
      ...
      vendorBlocked: blocked,
      reason: blocked ? 'vendor_usage_denied' : protectedByTrigger ? 'quota_reserve' : 'headroom_available' };
  }
  return { observe: observation => observe(observation),
    observeNativeCache: observation => observation?.provider === 'claude'
      && observation.source === 'claude_native_cache_v1' && observe(observation, true),
    bindIdentity, headroom, anchoredRolloverOccurred, fingerprint: (seat) => seats[seat]?.accountFingerprint || null, list: () => Object.keys(seats).map((key) => headroom(key)),
    ...
    verdict(quotaSeat, options) {
      const value = headroom(quotaSeat, options);
      let admit = !settings.usageProtection || !value.protected;
      // S5: headroom()/low is unchanged from main -- a window that is still reported "low"
      // keeps protecting the seat by percentRemaining alone. Admission is renewed here, at
      // the verdict layer, only when every window still pulling percentRemaining at/below
      // reserve has genuinely rolled over per the store's own anchored boundary (never a
      // raw/forged resetsAt), and the seat is not otherwise vendor-blocked.
      if (!admit && !value.vendorBlocked && Array.isArray(value.windows)) {
        const lowWindows = value.windows.filter((w) => typeof w.percentRemaining === 'number'
          && w.percentRemaining <= settings.reservePercent);
        if (lowWindows.length && lowWindows.every((w) => anchoredRolloverOccurred(w))) admit = true;
      }
      return { ...value, admit };
    } };
}
