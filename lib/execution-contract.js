...
function resolveProviderControls({ kind, entry = {}, registry, slot, taskTier, modelTier, model,
  ...
  const configFingerprint = fingerprint({ slot, mode, models: specs,
    ...
    effort_model_allowlist: entry.effort_model_allowlist || null,
  ...
  if (EXTREME_EFFORTS.has(requestedEffort) && phase !== 'plan' && maxEffortOverride !== true) fail('extreme_effort_requires_override', 'maxEffortOverride', 'Explicit xhigh/max/ultra effort requires maxEffortOverride=true.');
  ...
  // Hosted catalog authority is credential/config bound and checked live by
  // admission. The persisted CLI census cannot authorize or veto that account.
  const available = entry.oneshot_adapter === 'openai_chat_api' ? null : censusModels(registry, kind);
