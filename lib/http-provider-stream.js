...
function createOllamaStreamParser(options = {}) {
  ...
  // maxOutputBytes is a spill threshold, not a hard cap (Refs #133): past it,
  // deltas still accumulate in `outputBytes`/`spilling` and are handed to
  // options.onOutputSpill for the caller to persist, but only a bounded tail
  // (outputTailBytes) is kept in `output` for the in-memory `response` field.
  const outputTailBytes = Number.isSafeInteger(options.outputTailBytes) && options.outputTailBytes > 0
    ? options.outputTailBytes : 8 * 1024 * 1024;
  let wireBytes = 0, frameBytes = 0, outputBytes = 0, frames = 0, spilling = false;
  let fragments = [], outputTail = '', terminal = null, finished = false;
  ...
  function acceptFrame(text) {
    ...
    if (delta) {
      outputBytes += bytes;
      // Past maxOutputBytes, the stream still spills (never throws
      // output_cap, Refs #133): overflow deltas are handed to onOutputSpill
      // for the caller to persist to disk, while outputTail keeps only a
      // bounded tail in memory for the eventual `response` field, the same
      // convention as the CLI spawn output-spill path.
      if (outputBytes > limits.maxOutputBytes) {
        spilling = true;
        assertContinuing(options.onOutputSpill?.(delta));
      }
      outputTail += delta;
      if (outputTail.length > outputTailBytes) outputTail = outputTail.slice(-outputTailBytes);
  ...
  function finish() {
    ...
    return { ...terminal, response: outputTail, done: true,
      transport: { wireBytes, outputBytes, frames, spilling, limits: { ...limits } } };
  }
