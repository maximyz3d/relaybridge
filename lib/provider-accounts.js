...
const { own, safeDictionaryKey, validBaseQuotaSeat, validQuotaSeat } = require('./quota-seat');
...
function validAccountId(value) {
  return typeof value === 'string' && ACCOUNT_ID_RE.test(value) && !value.endsWith('.') && !WINDOWS_DEVICE_RE.test(value);
}

function validProviderKey(value) {
  return safeDictionaryKey(value) && PROVIDER_KEY_RE.test(value);
}
...
function validateRegistry(raw) {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)
    || !own(raw, 'providers') || !raw.providers || typeof raw.providers !== 'object' || Array.isArray(raw.providers)) {
  ...
  return { ...raw, providers: Object.assign(Object.create(null), raw.providers) };
}
...
function quotaSeatForAccount(baseQuotaSeat, accountId) {
  const base = baseQuotaSeat;
  if (!validBaseQuotaSeat(base)) throw new Error('invalid base quota seat');
  if (!validAccountId(accountId)) throw new Error('invalid account id');
  if (!accountId || accountId === DEFAULT_ACCOUNT_ID) return base;
  const quotaSeat = `${base}#${accountId}`;
  if (!validQuotaSeat(quotaSeat)) throw new Error('generated account quota seat is invalid');
  return quotaSeat;
}
...
function accountsFor(kind, entry, registry) {
  if (!validProviderKey(kind)) throw new Error('invalid provider key');
  const present = own(registry?.providers, kind);
  const row = present ? registry.providers[kind] : null;
  if (present && (!row || !own(row, 'accounts') || !Array.isArray(row.accounts))) {
    throw new Error('account registry provider must contain an accounts array');
  }
  const configured = present ? row.accounts : undefined;
  ...
  if (declaredBase && !safeDictionaryKey(declaredBase)) throw new Error('invalid base quota seat');
  const base = declaredBase && validBaseQuotaSeat(declaredBase) ? declaredBase : kind;
...
function noteAccountAuthFailure(dataDir, kind, accountId, entry) {
  ...
  const account = own(registry.providers, kind)
    ? registry.providers[kind].accounts.find((item) => item.id === accountId) : null;
...
function selectAccount({
  ...
  const quotaEligible = usable.filter((account) => {
    const gauge = own(gauges, account.quotaSeat) ? gauges[account.quotaSeat] : null;
    if (gauge?.basis === 'vendor_observed' && activeQualitativeQuotaExhaustion(gauge.vendorQuota)) return false;
  ...
  const scored = candidates.map((a) => {
    const gauge = own(gauges, a.quotaSeat) ? gauges[a.quotaSeat] : null;
...
function addAccount(dataDir, kind, { id, label }) {
  ...
  const provider = providerExisted ? registry.providers[kind] : (registry.providers[kind] = { accounts: [] });
...
function removeAccount(dataDir, kind, id) {
  ...
  const registry = loadRegistry(dataDir, { strict: true });
  const provider = own(registry.providers, kind) ? registry.providers[kind] : null;
...
function setAccountEnabled(dataDir, kind, id, enabled) {
  ...
  const registry = loadRegistry(dataDir, { strict: true });
  const account = own(registry.providers, kind)
    ? registry.providers[kind].accounts.find((a) => a.id === id) : null;
