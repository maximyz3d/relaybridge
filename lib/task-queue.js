...
function createTaskQueue(opts) {
  ...
  const deferred = new Set();  // queued tasks waiting for admission backoff
  let active = 0;
  ...
  async function runTask(task) {
    ...
    const requeueForAdmission = () => {
      ...
      const timer = setTimeout(() => {
        deferred.delete(current.id);
        if (read(current.id)?.status === 'queued') pending.push(current.id);
        pump();
      }, backoff);
  ...
  function stats() {
    return { active, queued: pending.length + deferred.size, maxConcurrent };
  }
