    ...
    try {
      await new Promise((resolve) => {
        ...
        Promise.resolve(opts.executeOneShot(task.body, res))
          .then(() => {
            // A handler that returned without responding would otherwise hold
            // its admission slot forever.
            if (!res.writableEnded) { res.__abort(); settle({ status: 'failed', error: 'provider handler returned without a response' }); resolve(); }
          })
          .catch((err) => {
            res.__abort();
