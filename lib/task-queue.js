...
const { requireCompleteText } = require('./instruction-integrity');
...
const { createOwnedTaskProjection, canonical: ownedCanonical } = require('./owned-task-projection');
...
// reports the rejection it kept getting. Unbounded by default: a task waits
// for a seat as long as it takes, and is refused only when the admission
// failure itself says the provider can never serve it (a different code path
// entirely — see the failureClass switch in runTask below, which never
// reaches requeueForAdmission for a missing/disabled provider because that
// fails immediately with its own failureClass, not a 429 admission_limit
// retry loop). opts.admissionWaitMs still overrides this for tests that need
// a bounded wait.
const ADMISSION_WAIT_MS = null;
...
function createTaskQueue(opts) {
  ...
  // null means unbounded: a queued task never expires purely from elapsed
  // admission-wait time.
  ...
  let active = 0;
  let stopped = false;
  let paused = opts.autoStart === false;
  const executionOwners = opts.executionOwners || null;
  if (executionOwners && ['listTaskReservations', 'readPreparedTaskOwner', 'readCommittedTaskRelease'].some((key) => typeof executionOwners[key] !== 'function')) throw new TypeError('incomplete private execution owner authority');
  const ownedProjection = executionOwners ? createOwnedTaskProjection(path.join(dir, 'owned-settlements')) : null;
  const ownedReferences = new Map(), ownedHeld = new Set(), executionContexts = new WeakSet();
  ...
  const ownerFailure = (code) => Object.assign(new Error(code), { code });
  function validateOwnerReference(row) {
    const keys = ['taskId', 'reservationId', 'ownerId', 'bindingHash', 'cwd', 'held', 'decisionId'];
    if (!row || Object.keys(row).sort().join('|') !== keys.sort().join('|')
      ...
      || typeof row.cwd !== 'string' || !path.isAbsolute(row.cwd) || typeof row.held !== 'boolean'
      || (row.held ? row.decisionId !== null : !/^[a-f0-9]{64}$/.test(row.decisionId))) throw ownerFailure('OWNED_TASK_REFERENCE_INVALID');
    return structuredClone(row);
  }
  function refreshOwnedReservations() {
    if (!executionOwners) return 0;
    const rows = executionOwners.listTaskReservations();
    if (!Array.isArray(rows) || rows.length > 2048) throw ownerFailure('OWNED_TASK_RESERVATIONS_UNCONFIRMED');
    const next = new Map(), held = new Set();
    for (const input of rows) {
      const row = validateOwnerReference(input);
      if (next.has(row.taskId)) throw ownerFailure('OWNED_TASK_RESERVATION_CONFLICT');
      if (!row.held) {
        const applied = ownedProjection.read(row.taskId);
        if (!applied || applied.ownerId !== row.ownerId || applied.reservationId !== row.reservationId
          || applied.bindingHash !== row.bindingHash || applied.decisionId !== row.decisionId) throw ownerFailure('OWNED_TASK_PROJECTION_UNCONFIRMED');
      } else held.add(row.taskId);
      next.set(row.taskId, row);
    }
    // Disappearance is never an implicit release. The private journal must
    // publish an applied decision with matching immutable task projection.
    for (const [id, row] of ownedReferences) if (!next.has(id)) { next.set(id, row); if (row.held) held.add(id); }
    ownedReferences.clear(); ownedHeld.clear();
    for (const [id, row] of next) ownedReferences.set(id, row);
    for (const id of held) ownedHeld.add(id);
    return ownedHeld.size;
  }
  function occupiedCount() { return new Set([...inFlight, ...uncertain, ...ownedHeld]).size; }
  function isExecutionContext(context) {
    return executionContexts.has(context) && inFlight.has(context.taskId)
      && read(context.taskId)?.executionReservation?.reservationId === context.reservationId;
  }
  function attachOwnedExecution(context, input) {
    if (!executionOwners || !isExecutionContext(context) || !input
      || Object.keys(input).sort().join('|') !== ['ownerId', 'bindingHash', 'reservationId'].sort().join('|')) throw ownerFailure('OWNED_TASK_CONTEXT_INVALID');
    const task = read(context.taskId), row = executionOwners.readPreparedTaskOwner(input.ownerId);
    if (!row || row.locallyPrepared !== true || row.started !== false) throw ownerFailure('OWNED_TASK_NOT_PREPARED');
    const reference = validateOwnerReference(row.reference);
    if (reference.taskId !== task.id || reference.reservationId !== context.reservationId || input.reservationId !== context.reservationId
      || reference.ownerId !== input.ownerId || reference.bindingHash !== input.bindingHash || !reference.held || ownedReferences.has(task.id)) throw ownerFailure('OWNED_TASK_BINDING_CHANGED');
    // The new task binding is durable before this returns to start/permit.
    write({ ...task, execution: { state: 'owned_held', owner: { ownerId: reference.ownerId,
      bindingHash: reference.bindingHash, reservationId: reference.reservationId } } });
    ownedReferences.set(task.id, reference); ownedHeld.add(task.id);
    return structuredClone(reference);
  }
  function projectOwnedTask(task) {
    if (!task) return task;
    const reference = ownedReferences.get(task.id);
    if (!reference) return task;
    return { ...task, execution: { ...task.execution, state: reference.held ? 'owned_held' : 'owned_settled',
      owner: { ownerId: reference.ownerId, bindingHash: reference.bindingHash, reservationId: reference.reservationId },
      ...(reference.held ? {} : { settlementEvidence: ownedProjection.read(task.id) }) } };
  }
  function applyOwnedTaskRelease(binding, decision) {
    if (!executionOwners || !binding || !decision) throw ownerFailure('OWNED_TASK_AUTHORITY_UNAVAILABLE');
    const reference = ownedReferences.get(binding.taskId), task = read(binding.taskId);
    if (!reference || !task || task.executionReservation?.reservationId !== reference.reservationId
      || binding.reservationId !== reference.reservationId || crypto.createHash('sha256').update(ownedCanonical(binding)).digest('hex') !== reference.bindingHash) throw ownerFailure('OWNED_TASK_BINDING_CHANGED');
    const authoritative = executionOwners.readCommittedTaskRelease(reference.ownerId, decision.decisionId);
    const expected = { version: 1, taskId: binding.taskId, reservationId: reference.reservationId,
      ownerId: reference.ownerId, bindingHash: reference.bindingHash, decisionId: decision.decisionId, scope: 'task_capacity' };
    if (!authoritative || ownedCanonical(authoritative) !== ownedCanonical(expected)) throw ownerFailure('OWNED_TASK_DECISION_UNTRUSTED');
    // Immutable sidecar only; never rewrite semantic terminal/legacy task bytes.
    // Keep capacity until the journal's own applied barrier confirms completion.
    return ownedProjection.apply(expected);
  }
  function resume() {
    if (stopped) throw new Error('task queue is shut down');
    refreshOwnedReservations(); paused = false; setImmediate(pump); return stats();
  }
  ...
  function write(task) {
    ...
    try { fs.writeFileSync(handle, JSON.stringify(task, null, 2), 'utf8'); fs.fsyncSync(handle); }
  ...
  function emitDelivery(task) {
    if (!task?.delivery || typeof opts.appendDeliveryReceipt !== 'function') return;
    try { for (const receipt of deliveryReceipts(task, opts.receiptStoreId)) opts.appendDeliveryReceipt(receipt); }
    catch { log(`[RelayBridge] tasks: delivery receipt pending for ${task.id}`); }
  }
  ...
  function reconcileOnStartup() {
    ...
    for (const name of fs.readdirSync(dir).filter((n) => /^t_[A-Za-z0-9_]+\.json$/.test(n))) {
      ...
      if (task.executionReservation || ownedReferences.has(task.id)) {
        if (task.startedAt != null && !ownedReferences.has(task.id)) uncertain.add(task.id);
        continue; // new owned records use immutable private projections, never legacy recovery rewrite
      }
  ...
  function dependencyState(task) {
    for (const id of task.dependsOn || []) {
      if (ownedHeld.has(id) || uncertain.has(id)) return 'dependency_physical_hold';
  ...
  function scheduleWake(referenceAt = now()) {
    if (wakeTimer !== null) clearLater(wakeTimer);
    wakeTimer = null;
    if (stopped) return;
    const due = [];
    for (const task of backlog.values()) {
      if (!dependencyState(task) && task.nextAttemptAt > referenceAt) due.push(task.nextAttemptAt);
      ...
      if (task.admissionDeadlineAt > referenceAt) due.push(task.admissionDeadlineAt);
    }
    if (due.length) {
  ...
  function pump() {
    ...
    // A deadline skipped as future during this pass must retain a wake even
    // if it becomes due before scheduleWake runs. Dispatch/expiry checks below
    // still use the live clock so a long pass cannot admit expired work.
    const wakeReferenceAt = now();
    ...
    for (const task of [...backlog.values()].sort((a, b) => a.createdAt - b.createdAt)) {
      ...
      if (blocked || task.nextAttemptAt > now() || occupiedCount() >= maxConcurrent) continue;
  ...
  function confirmStopped(id) {
    const task = read(id);
    if (task?.executionReservation || ownedReferences.has(id)) throw ownerFailure('OWNED_TASK_PRIVATE_DECISION_REQUIRED');
  ...
  async function runTask(task) {
    task.status = 'running';
    task.startedAt = now();
    task.execution = { state: task.executionReservation ? 'owned_unverified' : 'in_flight', dispatchedAt: now() };
    write(task);
    const context = Object.freeze({ taskId: task.id, reservationId: task.executionReservation?.reservationId || null,
      requiresOwner: !!task.executionReservation, get executionOwner() { const value = ownedReferences.get(task.id); return value ? structuredClone(value) : null; } });
    if (task.executionReservation) executionContexts.add(context);
    ...
    const requeueForAdmission = (payload) => {
      ...
      const deadlineAt = current.admissionDeadlineAt !== undefined ? current.admissionDeadlineAt
        : admissionWaitMs === null ? null : waitingSince + admissionWaitMs;
      if (deadlineAt !== null && now() >= deadlineAt) return false;   // waited long enough; report the rejection
      ...
      write({
        ...
        admissionDeadlineAt: deadlineAt,
        nextAttemptAt: deadlineAt === null ? now() + backoff : Math.min(deadlineAt, now() + backoff),
    ...
    try {
      await new Promise((resolve) => {
        const res = captureResponse((payload, statusCode) => {
          if (!task.executionReservation && statusCode === 429 && payload?.failureClass === 'admission_limit'
          ...
          const failed = statusCode >= 400 || nonzeroExit || noOutput
            ...
            || payload?.audit_usability?.usable === false || payload?.partial_result || payload?.failureClass || payload?.error || payload?.ok === false;
          const failureClass = payload?.failureClass || (failed ? (
            payload?.audit_usability?.usable === false ? 'ungrounded_audit' : payload?.budget_exceeded ? 'budget' : payload?.timed_out ? 'timeout'
          ...
          const ownerReference = ownedReferences.get(task.id);
          if (task.executionReservation && !ownerReference) uncertain.add(task.id);
          const execution = task.executionReservation
            ? { state: ownerReference ? 'owned_held' : 'owned_unverified', ...(ownerReference ? { owner: { ownerId: ownerReference.ownerId, bindingHash: ownerReference.bindingHash, reservationId: ownerReference.reservationId } } : {}) }
            : { state: payload?.model_invocation === false ? 'not_invoked' : 'settled', settledAt: now(), receiptId: payload?.receiptId || null };
          settle({
            status: failed ? 'failed' : 'done',
            execution,
            result: delivered ? delivered.text : clamp(text, MAX_RESULT_CHARS),
            resultIntegrity: { inputTruncated: payload?.prompt_truncated === true || payload?.route?.prompt_truncated === true
                || payload?.route?.prompt_evidence?.truncated === true, originalChars: String(text).length, storedChars: (delivered ? delivered.text : clamp(text, MAX_RESULT_CHARS)).length,
              truncated: payload?.stdout_truncated === true || payload?.stdoutTruncated === true
                || (delivered ? delivered.metadata.truncated === true : String(text).length > MAX_RESULT_CHARS) },
            ...
            providerMetadata: Object.fromEntries(['provider_run_id', 'token_usage_source', 'physical_attempt_count',
              ...
              'transport_lifecycle', 'process_census', 'process_warnings', 'native_transport', 'provider_state', 'result_subtype', 'continuity', 'grounding',
        ...
        Promise.resolve().then(() => opts.executeOneShot(task.body, res, context))
          .then(() => {
  ...
  function submitInternal(input, reservedId = null, delivery = null) {
    if (stopped) throw new Error('task queue is shut down');
    const prompt = requireCompleteText(String(input?.prompt ?? ''), MAX_PROMPT_CHARS, 'task prompt');
    ...
    const draft = {
      ...
      body: {
        ...
        ...(input.allowedWritePaths === undefined ? {} : { allowedWritePaths: input.allowedWritePaths }),
    ...
    if (executionOwners && typeof opts.requiresExecutionOwner === 'function' && opts.requiresExecutionOwner(structuredClone(draft)) === true) {
      draft.executionReservation = { version: 1, reservationId: 'qr_' + crypto.randomBytes(16).toString('hex') };
    }
    const task = write(draft);
  ...
  function stats() {
    ...
    return { active, queued: ready + deferred + blocked, maxConcurrent, ready, deferred, blocked,
      uncertain: new Set([...uncertain, ...ownedHeld]).size, ...(executionOwners ? { ownedHeld: ownedHeld.size, reserved: occupiedCount(), paused } : {}) };

  }

  refreshOwnedReservations(); // all restored holds precede every possible pump
  reconcileOnStartup();
  // Replay only delivery metadata for already persisted results, never work.
  for (const name of fs.readdirSync(dir)) {
    if (/^t_[A-Za-z0-9_]+\.json$/.test(name)) emitDelivery(read(name.slice(0, -5)));
  }
  if (backlog.size) setImmediate(pump);

  return { submit, submitReserved, submitDurable, getResult, acknowledgeResult: acknowledge,
    get: (id) => projectOwnedTask(read(id)), list, cancel, stats, shutdown, resume, isExecutionContext,
    attachOwnedExecution, applyOwnedTaskRelease, refreshOwnedReservations,
    unsettledInWorkspace(cwd) {
      for (const id of ownedHeld) if (ownedReferences.get(id)?.cwd === cwd) return true;
      return fs.readdirSync(dir).filter((name) => /^t_[A-Za-z0-9_]+\.json$/.test(name)).some((name) => {
        ...
        if (task.status !== 'queued' && !['in_flight', 'uncertain', 'owned_unverified', 'owned_held'].includes(task.execution?.state)) return false;
