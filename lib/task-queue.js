...
function createTaskQueue(opts) {
  ...
  function write(task) {
    ...
    if (task.status === 'queued') backlog.set(task.id, task);
  ...
  function reconcileOnStartup() {
    ...
    for (const name of fs.readdirSync(dir).filter((n) => /^t_[A-Za-z0-9_]+\.json$/.test(n))) {
      ...
      const eligible = waiting && task.schemaVersion === 2 && task.recovery?.mode === 'never-started'
        ...
        && Array.isArray(task.dependsOn) && Number.isSafeInteger(task.nextAttemptAt)
  ...
  function scheduleWake() {
    ...
    wakeTimer = null;
    if (stopped) return;
    const due = [...backlog.values()].filter((task) => !dependencyState(task) && task.nextAttemptAt > now())
  ...
  return { submit, submitReserved, get: read, list, cancel, stats, shutdown, confirmStopped, reconcileOnStartup, newTaskId, _pump: pump };
}
