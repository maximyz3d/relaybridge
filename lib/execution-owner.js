...
function createOwnerJournal({ directory, receiptStoreId, hostIdentity, qualifyHost,
  ...
  async function start(id, ...callerArguments) {
    ...
    if (validateLaunchAdmission(clone(state.binding), { ownerId: id, phase: 'start' }) !== true) throw error('OWNER_LAUNCH_ADMISSION_CHANGED');
  ...
  async function permit(id) {
    ...
    try {
      ...
      if (!validateCurrentBinding(clone(state.binding))) throw error('OWNER_BINDING_CHANGED');
      if (validateLaunchAdmission(clone(state.binding), { ownerId: id, phase: 'ready' }) !== true) throw error('OWNER_LAUNCH_ADMISSION_CHANGED');
      ...
      assertLaunchable(get(id));
      if (validateLaunchAdmission(clone(state.binding), { ownerId: id, phase: 'permit' }) !== true) throw error('OWNER_LAUNCH_ADMISSION_CHANGED');
