...
function createAttemptLifecycle({ runId, kind, route, supervisor, registry, releaseAdmission,
  ...
  schedule = setInterval, clearSchedule = clearInterval, strictSettlement = false } = {}) {
  ...
  if (typeof strictSettlement !== 'boolean') throw new TypeError('strictSettlement must be a boolean');
  ...
  let strictEvidence = null, strictCleanupComplete = false, strictCleanupResult = null;
  ...
  function sealOutcome(value) {
    ...
    drainDeadlineAt = Math.min(now() + drainTimeoutMs, supervisor.opts.hardDeadline === false
      ? Infinity : supervisor.startedAt + supervisor.opts.hardCapMs);
  ...
  function settlePhysical({ evidence, cleanup = async () => ({ ok: true, status: 'not_applicable' }),
    persist = null } = {}) {
    if (settlement) {
      if (strictSettlement && strictEvidence && evidence !== strictEvidence) throw new TypeError('strict settlement evidence cannot change');
      return settlement;
    }
    if (strictSettlement && (typeof persist !== 'function' || (strictEvidence && evidence !== strictEvidence))) {
      throw new TypeError('strict settlement requires an explicit matching durable persistence callback');
    }
    if (persist === null) persist = async () => null;
    ...
    if (strictSettlement) strictEvidence = evidence;
    phase = strictSettlement ? 'persisting' : 'cleanup';
    ...
    settlement = Promise.resolve().then(async () => {
      ...
      if (strictSettlement) {
        // The private caller must persist its exact owner proof before any
        // cleanup or admission/registry release. Readable bytes are not enough.
        try {
          persistenceResult = await persist({ outcome, snapshot: snapshot(), cleanup: null });
          if (persistenceResult?.durability !== 'confirmed') {
            throw Object.assign(new Error('durable physical settlement unconfirmed'), { code: 'LIFECYCLE_PERSISTENCE_UNCONFIRMED' });
          }
        } catch (error) {
          noteCallbackError(error); cleanupStatus = 'preserved_unconfirmed'; phase = 'settlement_blocked';
          settlement = null; // exact evidence may retry; physicalDone remains pending
          throw Object.assign(new Error('durable physical settlement unconfirmed'), { code: 'LIFECYCLE_PERSISTENCE_UNCONFIRMED' });
        }
        phase = 'cleanup';
        if (strictCleanupComplete) cleanupResult = strictCleanupResult;
        else {
          try {
            cleanupResult = await cleanup();
            cleanupStatus = cleanupResult?.ok === false ? 'failed_preserved' : 'complete';
          } catch (error) {
            noteCallbackError(error); cleanupStatus = 'failed_preserved'; cleanupResult = { ok: false, status: 'cleanup_failed_preserved' };
          }
          strictCleanupResult = cleanupResult; strictCleanupComplete = true;
        }
        // releaseAdmission must be the private idempotent admission projection.
        // If it fails, keep registry/finalization and allow an exact retry.
        try { await releaseAdmission(); }
        catch (error) {
          noteCallbackError(error); phase = 'settlement_blocked'; settlement = null;
          throw Object.assign(new Error('physical admission release unconfirmed'), { code: 'LIFECYCLE_RELEASE_UNCONFIRMED' });
        }
        if (registry.get(runId) === entry) registry.delete(runId);
        finalized = true; phase = 'settled';
        const result = { snapshot: snapshot(), cleanup: cleanupResult, persistence: persistenceResult };
        resolvePhysical(result); return result;
      }
      try {
        try {
          cleanupResult = await cleanup();
...
function normalizeTransportLifecycle(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    ...
    || !['admitted', 'running', 'stopping', 'draining', 'quarantined', 'persisting', 'settlement_blocked', 'cleanup', 'settled'].includes(value.phase)) return null;
  ...
  return { runId: value.runId, phase: value.phase, dispatched: value.dispatched === true,
    ...
    cleanupStatus: ['pending', 'complete', 'failed_preserved', 'quarantined_unverified', 'preserved_unconfirmed'].includes(value.cleanupStatus) ? value.cleanupStatus : null,
