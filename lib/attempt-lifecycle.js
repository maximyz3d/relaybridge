...
function createAttemptLifecycle({ runId, kind, route, supervisor, registry, releaseAdmission,
  ...
  function invokeTransportStop() {
    ...
    if (!requested || !transport || stopRequested || settlement) return;
    stopRequested = true;
  ...
  function requestStop({ reason, detail = '', source = 'caller' } = {}) {
    if (outcome || settlement || stop) return false;
  ...
  function bindTransport({ type, pid = null, requestStop: stopTransport } = {}) {
    if (transport || settlement) throw new Error('transport already bound or settled');
  ...
  function markDispatched() {
    if (settlement || outcome || stop || dispatched) return false;
  ...
  function identifyProcess(pid) {
    if (transport?.type !== 'cli' || settlement || !Number.isSafeInteger(pid) || pid <= 0
  ...
  function observeOutput(text, accept = () => {}) {
    if (!dispatched || stop || outcome || settlement) return false;
  ...
  function observeCpu(cpuMs) {
    if (!dispatched || stop || outcome || settlement || transport?.type !== 'cli') return false;
  ...
  const api = { bindTransport, markDispatched, identifyProcess, observeOutput, observeUsage, observeCpu,
    requestStop, clientDetached, sealOutcome, settlePhysical, snapshot, physicalDone, evaluate };
  entry.lifecycle = api;
...
function normalizeTransportLifecycle(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    ...
    || !['admitted', 'running', 'stopping', 'draining', 'cleanup', 'settled'].includes(value.phase)) return null;
  ...
  return { runId: value.runId, phase: value.phase, dispatched: value.dispatched === true,
    ...
    cleanupStatus: ['pending', 'complete', 'failed_preserved'].includes(value.cleanupStatus) ? value.cleanupStatus : null,
