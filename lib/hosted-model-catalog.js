...
function createHostedModelCatalog({ pool, now = Date.now, ttlMs = 60000, maxEntries = 128 } = {}) {
  const cache = new Map(), salt = crypto.randomBytes(32);
  async function check({ chatUrl, modelsUrl, key, model, generation = '', signal, currentIdentity = () => true }) {
    ...
    // Secret values participate only in this private in-memory HMAC. No key or
    // stable account fingerprint is logged, persisted or returned to a caller.
    const id = crypto.createHmac('sha256', salt).update(JSON.stringify([url.href, model, key.name, key.value, generation])).digest('hex');
