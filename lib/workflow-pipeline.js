  ...
  function acquireWriterLease(state, actor, requestedLeaseMs) {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      try {
        ...
        // Expiry is not termination evidence, including for legacy locks and
        // after restart. Recovery needs an explicit owner-termination protocol;
        // never manufacture permission to overlap writers from elapsed time.
        fail('WRITER_EXECUTION_UNCERTAIN', 'expired writer lease requires owner reconciliation; expiry does not prove termination', {
          cwd: state.cwd, runId: existing.runId, actor: existing.actor, expiresAt: existing.expiresAt,
        });
      }
    }
