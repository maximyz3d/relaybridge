...
const integrity = require('./instruction-integrity');
function requireCompleteText(...args) {
  try { return integrity.requireCompleteText(...args); }
  catch (error) { fail(error.code, error.message, error.details); }
}
function requireIntactArtifact(...args) {
  try { return integrity.requireIntactArtifact(...args); }
  catch (error) { fail(error.code, error.message, error.details); }
}
const fs = require('fs');
const path = require('path');
const { isDeepStrictEqual } = require('node:util');
const { createWorkflowOwnerProjection, PROTOCOL: OWNED_WRITER_PROTOCOL } = require('./workflow-owner-projection');
const OWNED_WRITER = Symbol('private owned writer enrollment');
const { SUPPORTED_EFFORTS: EFFORT_LEVELS } = require('./effort-controls');
...
const WORKFLOW_PROFILES = Object.freeze(['codex-claude', 'codex-astra-ultra']);
function phasePolicy(profile) {
  if (profile === 'codex-claude') return null;
  if (profile !== 'codex-astra-ultra') fail('INVALID_ARGUMENT', 'unknown workflow profile');
  const advisor = () => ({ provider: 'codex', model: 'gpt-6-astra', effort: 'ultra', maxEffortOverride: true });
  return { version: 1, planning: advisor(), review: advisor(), finalReview: advisor(),
    implementation: { mode: 'external', provider: 'codex' }, revision: { mode: 'external', provider: 'codex' } };
}
...
const TRANSITIONS = Object.freeze({
  ...
  implementing: Object.freeze(['implementation_ready', 'plan_ready', 'failed', 'cancelled']),
...
function createWorkflowPipeline(options = {}) {
  ...
  const executionOwners = options.executionOwners || null;
  let ownedWriterProjection = null;
  function requireOwnedAuthority() {
    if (!ownedWriterProjection) fail('OWNER_AUTHORITY_UNAVAILABLE', 'qualified owned writer authority is unavailable');
    executionOwners.assertHeld();
    return ownedWriterProjection;
  }
  function rejectOwnedRelease(state) {
    if (state.writerLease?.ownerProtocol) fail('OWNER_RELEASE_REQUIRED', 'owned writers require a committed owner release decision');
  }
  ...
  function validateState(state, expectedRunId) {
    ...
    if (![1, SCHEMA_VERSION].includes(state.schemaVersion)) fail('STATE_CORRUPT', 'unsupported workflow state schema');
    ...
    // Only genuinely historical records may omit both policy fields. A broken
    // explicit policy must never degrade to a different provider after restart.
    if (state.schemaVersion === SCHEMA_VERSION || state.profile !== undefined || state.phasePolicy !== undefined) {
      if (!WORKFLOW_PROFILES.includes(state.profile)
        || !isDeepStrictEqual(state.phasePolicy, phasePolicy(state.profile))) {
        fail('STATE_CORRUPT', 'workflow phase policy is invalid');
      }
    }
    if (state.writerLease?.mode != null && !['external', 'provider'].includes(state.writerLease.mode)) {
      fail('STATE_CORRUPT', 'writer ownership mode is invalid');
    }
    if (state.phasePolicy && state.writerLease && state.writerLease.mode !== 'external') {
      fail('STATE_CORRUPT', 'writer ownership contradicts the external-only phase policy');
    }
    if (state.writerLease?.mode === 'external' && state.providerTask) {
      fail('STATE_CORRUPT', 'external writer cannot own a provider task');
    }
    if (state.phasePolicy && state.providerTask) {
      const binding = state.providerTask;
      const policy = state.phasePolicy[binding.purpose === 'final-review' ? 'finalReview' : binding.purpose];
      if (binding.phase !== state.phase || binding.purpose !== PROVIDER_TASK_PHASES[state.phase]
        || !policy || policy.mode || policy.provider !== binding.provider || policy.model !== binding.model || policy.effort !== binding.effort) {
        fail('STATE_CORRUPT', 'provider binding contradicts the persisted phase policy');
      }
    }
  ...
  function writeArtifact(runId, kind, markdown) {
    requireCompleteText(markdown, artifactCaps[kind], kind);
    atomicWriteFile(artifactPath(runId, kind), markdown);
    return {
      kind,
  ...
  function parseLock(filePath, expectedCwd, { allowMissingRace = false } = {}) {
    let lock;
    try {
      const stat = fs.lstatSync(filePath);
      ...
      if (error instanceof WorkflowPipelineError) throw error;
      if (allowMissingRace && error.code === 'ENOENT') throw error;
      fail('LOCK_CORRUPT', `writer lock is unreadable and will not be reclaimed: ${error.code || error.message}`);
    }
    if (!lock || lock.schemaVersion !== LOCK_SCHEMA_VERSION || !RUN_ID_RE.test(String(lock.runId || ''))
  ...
  function acquireWriterLease(state, actor, requestedLeaseMs) {
    // Restored journal holds must block ordinary as well as owned successor claims.
    if (executionOwners) { executionOwners.assertHeld(); executionOwners.assertWorkspaceAvailable(state.cwd); }
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      try {
        ...
        let existing;
        try { existing = parseLock(filePath, state.cwd, { allowMissingRace: true }); }
        catch (readError) {
          // An authorized release may win between EEXIST and this read.
          // Retry creation only on disappearance, never on age or corruption.
          if (readError.code === 'ENOENT') continue;
          throw readError;
        }
        const expired = existing.expiresAt <= currentTime();
        // Expiry revokes the holder's capability; it does not prove that its
        // process or descendants stopped. Preserve the lock until a separate
        // evidence-bound recovery operation can reconcile ownership/effects.
        fail(expired ? 'WRITER_EXECUTION_UNCERTAIN' : 'WRITER_CONFLICT',
          expired ? 'expired writer ownership remains held; recovery evidence is required'
            : 'another workflow holds the writer lease for this workspace', {
            cwd: state.cwd, runId: existing.runId, actor: existing.actor,
            expiresAt: existing.expiresAt,
            ...(expired ? { expired: true, recovery: 'unavailable_unbound_owner' } : {}),
          });
      }
    }
  ...
  function beginWriterPhase(runId, expectedPhase, targetPhase, event, input, fallbackActor, ownership = null) {
    if (ownership === OWNED_WRITER) requireOwnedAuthority();
    assertPlainObject(input, 'writer phase options');
    const state = requireState(runId);
    for (const [kind, record] of Object.entries(state.artifacts)) requireIntactArtifact(record, kind);
    requirePhase(state, expectedPhase, event);
    const actor = assertActor(input.actor, fallbackActor);
    const mode = enumValue(input.mode, 'external', ['external', 'provider'], 'writer mode');
    if (state.phasePolicy && mode !== 'external') fail('INVALID_WRITER_MODE', 'this profile requires an external writer');
    ...
    state.writerLease = {
      ...
      ...(ownership === OWNED_WRITER ? { ownerProtocol: OWNED_WRITER_PROTOCOL,
        ownerEpoch: `lease_${crypto.randomBytes(24).toString('hex')}` } : {}),
    };
    try {
  ...
  function finishWriterPhase(
    ...
    requireCompleteText(markdown, artifactCaps[artifactKind], artifactKind);
  ...
  function createWorkflow(input) {
    ...
    for (const [kind, markdown] of Object.entries(scope)) requireCompleteText(markdown, artifactCaps[kind], kind);
    ...
    try {
      ...
      const state = {
        ...
        profile: enumValue(input.profile, 'codex-claude', WORKFLOW_PROFILES, 'profile'),
        phasePolicy: phasePolicy(input.profile || 'codex-claude'),
      ...
      if (state.phasePolicy && Object.values(state.providerPreferences).some((providers) => providers.some((p) => p !== 'codex'))) {
        fail('INVALID_ARGUMENT', 'provider preferences contradict the Codex-only profile');
      }
  ...
  function updateScope(runId, input) {
    ...
    const validated = supplied.map(([field, kind, required]) => {
      const markdown = normalizedMarkdown(input[field], field, { required });
      requireCompleteText(markdown, artifactCaps[kind], kind);
      return [kind, markdown];
    });
    const hashes = {};
    for (const [kind, markdown] of validated) {
      hashes[kind] = installArtifact(state, kind, markdown).sha256;
    }
  ...
  function completePlanning(runId, input) {
    ...
    requireCompleteText(markdown, artifactCaps['plan'], 'plan');
    archiveProviderTask(state, 'completed', actor);
  ...
  function checkpointAndRelease(runId, input) {
    const state = requireState(runId);
    requirePhase(state, 'implementing', 'checkpointAndRelease');
    if (state.writerLease?.mode !== 'external') fail('INVALID_WRITER_MODE', 'managed writers require physical settlement through their controller');
    // A holder explicitly relinquishes unfinished work; this never claims that
    // implementation completed and never advances to review.
    return finishWriterPhase(runId, 'implementing', 'plan_ready', 'implementation_yielded', 'handoff', input, state.writerLease.actor);
  }
  ...
  function completeReview(runId, input) {
    ...
    requireCompleteText(markdown, artifactCaps['review'], 'review');
    archiveProviderTask(state, 'completed', actor);
  ...
  function startRevision(runId, input = {}, ownership = null) {
    const state = requireState(runId);
    for (const [kind, record] of Object.entries(state.artifacts)) requireIntactArtifact(record, kind);
    ...
    return beginWriterPhase(runId, 'review_ready', 'revising', 'revision_started', input, 'codex', ownership);
  }
  ...
  function completeFinalReview(runId, input) {
    ...
    requireCompleteText(markdown, artifactCaps['final-review'], 'final-review');
    archiveProviderTask(state, 'completed', actor);
  ...
  function bindProviderTask(runId, input) {
    ...
    if (state.writerLease?.mode === 'external') fail('INVALID_WRITER_MODE', 'an external writer cannot bind a provider task');
    if (!expectedPurpose) {
    ...
    const model = input.model == null ? null : String(input.model);
    if (model != null && !/^[A-Za-z0-9][A-Za-z0-9_.:/-]{0,199}$/.test(model)) fail('INVALID_PROVIDER_TASK', 'invalid model');
    const policy = state.phasePolicy?.[purpose === 'final-review' ? 'finalReview' : purpose];
    if (state.phasePolicy && (!policy || policy.provider !== provider || policy.model !== model || policy.effort !== effort)) {
      fail('INVALID_PROVIDER_TASK', 'provider task contradicts the persisted phase policy');
    }
  ...
  function finishBoundWriterTask(runId, input, status) {
    rejectOwnedRelease(requireState(runId));
  ...
  function finishOrphanedRevision(runId, input, status) {
    ...
    if (state.phasePolicy?.revision.mode === 'external' || state.writerLease?.mode !== 'provider') {
      fail('WRITER_EXECUTION_UNCERTAIN', 'only an explicitly provider-owned dispatch gap can use orphan recovery');
    }
  ...
  function renewWriterLease(runId, input) {
    ...
    const ownerLease = requireState(runId).writerLease;
    if (ownerLease?.ownerProtocol) { requireOwnedAuthority(); executionOwners.assertMutableLease(ownerLease.ownerEpoch); }
    const state = requireState(runId);
    ...
    for (const name of fs.readdirSync(workflowsDir)) {
      ...
      try {
        ...
        rows.push({
          ...
          cwd: state.cwd,
          phase: state.phase,
          profile: state.profile || 'codex-claude',
          createdAt: state.createdAt,
          updatedAt: state.updatedAt,
          revision: state.revision,
          writer: state.writerLease ? {
            actor: state.writerLease.actor,
            mode: state.writerLease.mode || 'unknown',
  ...
  if (executionOwners) ownedWriterProjection = createWorkflowOwnerProjection({
    authority: executionOwners, requireState, statePath, lockIdentity, canonicalizeCwd,
    addHistory, archiveProviderTask, installArtifact, artifactPath, revisionArtifactCap: artifactCaps.revision,
    verifyArtifacts: (state, pendingRevisionSha256) => {
      for (const [kind, record] of Object.entries(state.artifacts)) {
        requireIntactArtifact(record, kind);
        try { readArtifact(state.runId, kind); } catch (error) {
          // A prior attempt may have published this decision's replacement
          // revision artifact before its state transaction failed.
          if (kind !== 'revision' || error.code !== 'ARTIFACT_INTEGRITY'
            || sha256(fs.readFileSync(artifactPath(state.runId, kind), 'utf8')) !== pendingRevisionSha256) throw error;
        }
      }
    },
    now: currentTime, fsApi: options.executionOwnerFs,
  });

  // Internal launch/recovery seams. No REST/MCP endpoint is enabled by these.
  function startOwnedRevision(runId, input = {}) {
    requireOwnedAuthority();
    assertPlainObject(input, 'owned revision');
    if (Object.keys(input).some((key) => !['actor', 'leaseMs'].includes(key))) fail('INVALID_ARGUMENT', 'unknown owned revision field');
    return startRevision(runId, { ...input, mode: 'provider' }, OWNED_WRITER);
  }

  // A new owned lease fences its workspace before task/journal enrollment.
  // Reading this identity neither adopts legacy leases nor releases a lock.
  function ownedWorkspaceHeld(cwd) {
    const canonicalCwd = canonicalizeCwd(cwd), { filePath } = lockIdentity(canonicalCwd);
    let lock;
    try { lock = parseLock(filePath, canonicalCwd, { allowMissingRace: true }); }
    catch (error) { if (error.code === 'ENOENT') return false; throw error; }
    const state = requireState(lock.runId), lease = state.writerLease;
    if (lease?.ownerProtocol !== 'linux_pid1_writer_v1') return false;
    if (state.cwd !== canonicalCwd || lease.actor !== lock.actor || lease.leaseTokenSha256 !== sha256(lock.leaseToken)
      || lease.acquiredAt !== lock.acquiredAt || lease.expiresAt !== lock.expiresAt) {
      fail('LEASE_MISMATCH', 'owned writer state does not match its workspace lock');
    }
    return true;
  }
  ...
  return Object.freeze({
    ownedWorkspaceHeld,
    startOwnedRevision,
    getOwnedWriterBinding: (runId) => requireOwnedAuthority().exactLease(requireState(runId)),
    bindOwnedWriterExecution: (runId, input) => requireOwnedAuthority().bind(runId, input),
    applyOwnedWriterRecovery: (runId, input) => requireOwnedAuthority().apply(runId, input),
    applyOwnedWriterFinalization: (runId, input) => requireOwnedAuthority().finalize(runId, input),
    createWorkflow,
    updateScope,
    completeResearch,
    startPlanning,
