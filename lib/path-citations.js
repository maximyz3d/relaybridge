...
function verifyReferencedPaths(output, cwd, { minPaths = 1 } = {}) {
  ...
  const present = [], missing = [], citations = [];
  if (referenced.length >= minPaths) for (const raw of referenced) {
    ...
    const absolute = path.resolve(root, normalized.value);
    const canonical = inside(root, absolute) ? canonicalTarget(absolute) : { target: absolute };
    ...
    citations.push({ reference: raw, status, ...(['present', 'missing'].includes(status)
      ? { relativePath: path.relative(root, canonical.target) } : {}) });
    if (status === 'present') present.push(raw);
    else if (status === 'missing') missing.push(raw);
  }
  const total = present.length + missing.length;
  const confidence = !total ? 'no-paths-cited' : !present.length ? 'likely-fabricated'
    : missing.length > present.length ? 'suspect' : missing.length ? 'partial' : 'ok';
