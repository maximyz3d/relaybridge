...
function extractReferencedPaths(text, limit = 60) {
  ...
  for (const match of source.matchAll(/[^\s`"'<>()[\]{},;!?]{1,2048}/g)) {
    let token = match[0].replace(/[.]+$/, '');
    const doubleWrapped = token.startsWith('**') || token.endsWith('**');
    const wrapped = token.startsWith('*') || token.endsWith('*');
    if (doubleWrapped) {
      if (!(token.startsWith('**') && token.endsWith('**') && token.length > 4)) continue;
      token = token.slice(2, -2);
    } else if (wrapped) {
      if (!(token.startsWith('*') && token.endsWith('*') && token.length > 2)) continue;
