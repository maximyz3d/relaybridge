...
const { taskFailureDetails } = require('./incident-log');
  ...
  function reportIncident(workflow, task, details) {
    ...
    try {
      incidents.report({
        ...
        correlation: {
          requestId: workflow.runId,
          invocationId: task?.id || null,
          attemptId: task?.id ? `${task.id}:attempt:${attempt}` : null,
      ...
      log(`[RelayBridge] workflow ${workflow.runId}: incident logging failed: ${error.message}`);
    }
  }
