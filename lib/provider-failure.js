...
const { parseGeminiQuotaExhaustion } = require('./vendor-quota');
...
// Authentication is account authority, so these are deliberately anchored to
// provider-level diagnostic lines. CLIs often merge auxiliary MCP/plugin
// warnings into stderr; a line such as "MCP server docs: authentication
// required" must not quarantine the provider account after an unrelated
// non-zero exit.
const AUTH_FAILED = [
  /^\s*(?:(?:api\s+)?error:\s*)?(?:http\s*)?(?:401(?:\s+unauthori[sz]ed)?|403\s+unauthori[sz]ed)[.!]?\s*$/im,
  /^\s*(?:error:\s*)?(?:you are\s+)?not (?:authenticated|logged in|signed in)[.!]?(?:\s+please (?:run\s+)?(?:\/login|\S+\s+(?:auth\s+)?login)[.!]?)?\s*$/im,
  /^\s*(?:error:\s*)?(?:authentication|login|sign-in) (?:failed|required)[.!]?\s*$/im,
  /^\s*(?:error:\s*)?invalid (?:api )?(?:key|credential|token)[.!]?\s*$/im,
  /^\s*(?:error:\s*)?please run\s+(?:`[^`]*(?:login|auth)[^`]*`|\/login)[.!]?\s*$/im,
  /^\s*(?:error:\s*)?oauth (?:credential|token|session) (?:was\s+)?(?:revoked|expired)[.!]?\s*$/im,
];

// Copilot 1.0.x uses these provider-level terminal lines. Keep them gated by
// provider identity so an MCP/plugin warning containing the same words cannot
// quarantine another CLI's account.
const COPILOT_AUTH_FAILED = [
  /^\s*Error:\s*No authentication information found\.\s*$/im,
  /^\s*Not authenticated\. Please authenticate first\.\s*$/im,
  /^\s*Error:\s*[^\r\n]{1,240}\s+requires authentication\. Please sign in first with:\s*copilot login\.?\s*$/im,
];

// The bare word "capacity" used to be an overload signature. It matched any
// run that died for an unrelated reason while its output discussed capacity
// planning — classifyRunFailure({stdout: 'Plan the disk capacity for the
// cluster.', exitCode: 1}) came back as 'overloaded' — which cooled a healthy
// seat for 60s and hid the real cause behind the wrong failureKind in the
// ledger. Only the phrases that actually appear in overload responses count.
const OVERLOADED = [
  /\b529\b/, /\b503\b/, /overloaded/i, /server is busy/i,
  /service unavailable/i, /(?:at|over|insufficient) capacity/i,
];
...
const HOSTED_API_KEY_MISSING_CODE = 'RELAYBRIDGE_HOSTED_API_KEY_MISSING';

function isHostedApiKeyMissingError(error) {
  return !!error && error.code === HOSTED_API_KEY_MISSING_CODE;
}

// HTTP 401 is authentication evidence. HTTP 403 is not: providers also use it
// for quota, model/org policy and permission failures. Classify the response
// detail before allowing it to alter durable account authority.
function classifyProviderHttpFailure(status, detail = '') {
  const code = Number(status);
  const text = String(detail || '');
  if (code === 401) return 'auth';
  if (code === 403) {
    if (matches(RATE_LIMITED, text)) return 'rate_limit';
    if (/\b(?:budget|credit)s?\b/i.test(text)) return 'budget';
    if (matches(AUTH_FAILED, text)) return 'auth';
    if (/\b(?:forbidden|permission denied|access denied|not authori[sz]ed|does not have access)\b/i.test(text)) {
      return 'permission';
    }
  ...
  if (code === 402) return 'budget';
  return Number.isFinite(code) ? 'provider_error' : null;
}

const COPILOT_MONTHLY_QUOTA_LINE = /^\s*You have exceeded your monthly quota(?:\s+\(Request ID:\s*[A-F0-9:]{3,160}\))?\s*$/im;

const CURSOR_NAMED_MODELS_UNAVAILABLE_LINE = /^\s*ActionRequiredError:\s*Named models unavailable Free plans can only use Auto\. Switch to Auto or upgrade plans to continue\.\s*$/im;
const CURSOR_USAGE_LIMIT_LINE = /^\s*ActionRequiredError:\s*You've hit your usage limit Get Cursor Pro for more Agent usage, unlimited Tab, and more\.\s*$/im;

/**
 * Recognize only the two Cursor ActionRequiredError diagnostics observed in
 * #71. Provider identity, a failed exit, and an exact transport line are all
 * required. The named-model remedy is attributed to RelayBridge only when the
 * actual command carried a model flag; otherwise the CLI/account default is
 * the unresolved source and an automatic retry could repeat the same failure.
 */
function detectCursorActionRequired(run = {}) {
  if (String(run.provider || '').toLowerCase() !== 'cursor') return null;
  if (run.exitCode === 0) return null;
  const stdout = String(run.stdout || '');
  const stderr = String(run.stderr || '');
  const namedSource = CURSOR_NAMED_MODELS_UNAVAILABLE_LINE.test(stderr)
    ? 'cursor_cli_stderr' : CURSOR_NAMED_MODELS_UNAVAILABLE_LINE.test(stdout)
      ? 'cursor_cli_stdout' : null;
  if (namedSource) {
    return {
      provider: 'cursor',
      kind: 'named_models_unavailable',
      scope: 'plan',
      source: namedSource,
      diagnostic: 'Named models unavailable; Free plans can only use Auto',
      modelFlagSent: run.modelFlagSent === true,
    };
  }
  const usageSource = CURSOR_USAGE_LIMIT_LINE.test(stderr)
    ? 'cursor_cli_stderr' : CURSOR_USAGE_LIMIT_LINE.test(stdout)
      ? 'cursor_cli_stdout' : null;
  if (usageSource) {
    return {
      provider: 'cursor',
      kind: 'usage_quota_exhausted',
      scope: 'seat',
      source: usageSource,
...
function isLiteralOutputRequest(prompt, output) {
  const text = String(prompt || '');
  // Translation cannot be matched against the source language. These narrow
  // semantic detectors must not discard correctly transformed user content.
  return /^\s*(?:please\s+)?translate\b/i.test(text)
    || (/^\s*(?:please\s+)?(?:quote|repeat|print|output)\b/i.test(text)
      && text.includes(String(output || '').trim()));
}
...
function isNarrationOnlyResponse({ prompt = '', stdout = '' } = {}) {
  ...
  if (!text || text.length > 2400 || isPlanningRequest(prompt) || isLiteralOutputRequest(prompt, text)) return false;
...
function outputDetector(id, output) {
  return { id, version: 1, source: 'provider_terminal_text', outputChars: output.length,
    outputHash: crypto.createHash('sha256').update(output).digest('hex') };
}

// These are whole-output, provider-specific signatures, not keyword searches.
// Quoted examples, code, caution followed by findings, and substantive answers
// must remain usable. A missed unfamiliar phrase is safer than discarding a
// genuine result based on loose refusal/progress vocabulary.
const GEMINI_REFUSAL = /^(?:(?:I'm |I am )?sorry,?\s+(?:but\s+)?)?I (?:cannot|can't|am unable to) (?:fulfill your request|assist with (?:that|this|your) request|help with (?:that|this|your) request)[.!]?(?:\s+I am unable to perform adversarial reviews, vulnerability scanning, or security analysis on user-provided codebases[.!]?)?$/i;
const GEMINI_PROGRESS = /^I(?: am|['’]m| will|['’]ll) (?:downloading|fetching|retrieving|reading|searching|reviewing|checking|examining|analysing|analyzing|gathering|inspecting|download|fetch|retrieve|read|search|review|check|examine|analyse|analyze|gather|inspect) [^.!?:;\r\n]{1,300}(?:[.!…]+|$)(?:\s+(?:(?:This|It) (?:should|will) (?:just )?take (?:a moment|a few moments|a minute)|Please (?:wait|bear with me))[.!…]*)?$/i;
const COMPLETED_RESULT = /\b(?:found|confirmed|verified|completed|implemented|fixed|changed|updated|created|here\s+(?:is|are))\b/i;

function classifyGeminiTerminalText(run = {}) {
  if (!['gemini', 'gemini_cli', 'antigravity'].includes(String(run.provider || '').toLowerCase()) || run.exitCode !== 0) return null;
  const output = String(run.stdout || '');
  const text = output.trim();
  if (!text || text.length > 1200 || /```|~~~/.test(text) || /^[\s]*["'“‘>{\[|]/.test(text)) return null;
  const normalized = text.replace(/\s+/g, ' ');
  const prompt = String(run.prompt || '');
  if (isLiteralOutputRequest(prompt, text)) return null;
  if (GEMINI_REFUSAL.test(normalized)) return {
    kind: 'provider_refusal', resultSubtype: 'provider_refusal', retryable: false, failUp: true,
    detail: 'the provider explicitly declined the task; this is not completed review evidence. Narrow the task or use another eligible provider; do not repeat the unchanged call on this seat',
    partialResult: true, partialDiagnostic: output,
    outputDetector: outputDetector('gemini_explicit_refusal', output),
  };
  if (!isPlanningRequest(prompt) && GEMINI_PROGRESS.test(normalized) && !COMPLETED_RESULT.test(normalized)) return {
    kind: 'incomplete_response', resultSubtype: 'progress_only_completion', retryable: false, failUp: true,
    detail: 'the provider returned only a progress message, not a task result. Narrow the task or switch providers; do not retry the unchanged call on this seat',
    partialResult: true, partialDiagnostic: output,
    outputDetector: outputDetector('gemini_progress_only', output),
  };
  return null;
}
...
function classifyRunFailure(run = {}) {
  ...
  const quotaEvidence = parseGeminiQuotaExhaustion(run);
  if (quotaEvidence) return { kind:'rate_limited', retryable:false, failUp:true, quotaEvidence,
    detail:'Gemini reported individual quota exhaustion. Use another eligible account until the recorded reset; subscription allowance is unknown.' };
  ...
  const geminiTerminal = classifyGeminiTerminalText(run);
  if (geminiTerminal) return geminiTerminal;
  ...
  if (exitCode === 0 && isNarrationOnlyResponse(run)) {
    return { kind: 'incomplete_response', retryable: false, failUp: true,
      ...
      resultSubtype: 'progress_only_completion', partialResult: true, partialDiagnostic: String(run.stdout || ''),
      outputDetector: outputDetector('future_narration_only', String(run.stdout || '')) };
  }
  ...
  const cursorActionRequired = detectCursorActionRequired(run);
  if (cursorActionRequired?.kind === 'usage_quota_exhausted') {
    return {
      kind: 'budget', retryable: false, failUp: true,
      detail: 'Cursor Agent usage quota is exhausted; cool this seat and route elsewhere instead of retrying it',
      actionRequired: cursorActionRequired,
    };
  }
  if (cursorActionRequired?.kind === 'named_models_unavailable') {
    return {
      kind: cursorActionRequired.modelFlagSent ? 'plan_restriction' : 'provider_error',
      retryable: false,
      failUp: true,
      detail: cursorActionRequired.modelFlagSent
        ? 'Cursor Free rejected the named model sent by RelayBridge; remove the model flag so Cursor Auto/account-default selection can apply'
        : 'Cursor reported a named-model restriction even though RelayBridge sent no model flag; inspect the Cursor CLI/account default before retrying',
      actionRequired: cursorActionRequired,
    };
  }
  ...
  if (matches(AUTH_FAILED, text)
      || (String(run.provider || '').toLowerCase() === 'copilot' && matches(COPILOT_AUTH_FAILED, text))) {
...
// Interrupted streams contain prompts, tool inputs and partial model prose.
// They are diagnostic artifacts, not provider account-authority evidence.
function vendorEvidenceText({ stderr = '', stdout = '', diagnostic = '', includeStdout = false, supervisorStopReason } = {}) {
  return [stderr, includeStdout && !supervisorStopReason ? stdout : '', diagnostic].filter(Boolean).join('\n');
}

function receiptFailureKind({ supervisorStopReason, failureClass, apiErrorStatus, rateLimited, authFailed, actionRequiredKind, classifiedKind } = {}) {
  // An actual structured HTTP error remains evidence even if a supervisor
  // subsequently stopped the process; narrative substrings do not.
  if (Number(apiErrorStatus) === 429) return 'rate_limited';
  if (supervisorStopReason) return failureClass || `supervisor_${supervisorStopReason}`;
  if (actionRequiredKind === 'usage_quota_exhausted') return 'quota_exhausted';
  if (rateLimited) return 'rate_limited';
  if (authFailed) return 'auth_failed';
  return failureClass || (classifiedKind !== 'ok' ? classifiedKind : null);
}
...
module.exports = {
  ...
  CURSOR_NAMED_MODELS_UNAVAILABLE_LINE,
  CURSOR_USAGE_LIMIT_LINE,
  COPILOT_AUTH_FAILED,
