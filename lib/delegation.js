...
const INVOCATION_FIELDS = ['cwd', 'requiresWorkspaceAccess', 'inlineEvidence', 'maxEffortOverride',
  'expectedCwdIdentityHash', 'expectedCwdPolicyId', 'timeoutMs'];
const copy = (value) => JSON.parse(JSON.stringify(value));
function executionIntent(execution, invocation = {}) {
  const result = { invocation: {} };
  if (execution != null) result.execution = copy(execution);
  for (const key of INVOCATION_FIELDS) if (invocation[key] !== undefined) result.invocation[key] = copy(invocation[key]);
  return result;
}
function delegationTaskTier(task, classification = {}) {
  const classified = TASK_TIERS.includes(classification.tier) ? classification.tier : 'standard';
  const declared = text(task.taskTier).toLowerCase();
  return TASK_TIERS.includes(declared) && TIER_RANK[declared] <= TIER_RANK[classified] ? declared : classified;
}
...
function createDelegationCoordinator(opts = {}) {
  ...
  function bodyForEntry(record, entry, user, source = 'delegation') {
    const contract = entry.contract;
    const execution = entry.executionIntent?.execution;
    const requestedEffort = execution ? execution.requestedEffort
      : Object.hasOwn(entry, 'replanRequestedEffort') ? entry.replanRequestedEffort : contract.model.effort;
    const controls = execution ? {
      execution: copy(execution),
      ...(execution.model == null ? {} : { model: execution.model }),
      taskTier: execution.resolvedTaskTier,
      modelTier: execution.resolvedModelTier,
    } : { taskTier: contract.taskTier, modelTier: contract.model.modelTier };
    if (requestedEffort != null) controls.effort = requestedEffort;
    return {
      ...copy(entry.executionIntent?.invocation || {}), ...controls,
      kind: entry.provider,
      prompt: `${renderHandoffBrief(contract)}\n\n---\n\n${entry.prompt}`,
      cwd: contract.cwd, dangerous: contract.permissions.dangerous,
      providerBudget: entry.providerBudget,
      maxEffortOverride: entry.executionIntent?.invocation?.maxEffortOverride === true,
      user, source, title: entry.title,
      intent: `${record.delegationId} ${entry.taskRef}; ${contract.objective.slice(0, 160)}`,
    };
  }
  function prepareBody(record, entry, user, source) {
    const body = bodyForEntry(record, entry, user, source);
    const prepared = typeof opts.prepareTaskBody === 'function' ? opts.prepareTaskBody(body) : body;
    if (!prepared || prepared.kind !== entry.provider || prepared.cwd !== entry.contract.cwd
      || typeof prepared.prompt !== 'string' || prepared.prompt !== body.prompt) {
      fail('INVALID_PREPARED_TASK', 'Task preparation must retain provider, workspace and complete prompt.');
    }
    return prepared;
  }
  ...
  function delegate(input = {}) {
    ...
    const classified = tasks.map((task, index) => {
      if (!task || typeof task !== 'object' || Array.isArray(task)) fail('INVALID_ARGUMENT', `tasks[${index}] must be an object`);
      if (typeof (task.prompt || task.objective) !== 'string') fail('INVALID_ARGUMENT', `tasks[${index}] needs a prompt (string)`);
      ...
      const tier = delegationTaskTier(task, classification);
      return {
        index,
        task,
        prompt,
    ...
    for (const item of rank(classified)) {
      ...
      const contract = buildHandoffContract({
        ...
        cwd: selection.invocation?.cwd || task.cwd || input.cwd || process.cwd(),
      ...
      entries.push({
        ...
        executionIntent: executionIntent(selection.execution, selection.invocation),
        outcome: null,
        escalation: null,
        blockedReason: null,
      });
    ...
    // Validate every complete rendered task before any task can become runnable.
    const prepared = record.entries.filter((entry) => entry.status === 'planned').map((entry) => {
      const body = prepareBody(record, entry, actor);
      entry.executionIntent = executionIntent(body.execution, body);
      return { entry, body };
    });
    write(record);
    for (const { entry, body } of prepared) submitEntry(record, entry, body);
    record.updatedAt = new Date(now()).toISOString();
    write(updateStatus(record));
  ...
  function decideEscalation(delegationId, taskRef, decision = {}) {
    ...
    const modelChanged = widened.model.modelTier !== entry.contract.model.modelTier
      || widened.model.effort !== entry.contract.model.effort || widened.taskTier !== entry.contract.taskTier;
    const needsReplan = !!entry.executionIntent?.execution && modelChanged;
    const replanRequestedEffort = needsReplan && !resolved.kinds.includes('effort')
      ? entry.executionIntent.execution.requestedEffort : widened.model.effort;
    const candidate = { ...entry, contract: widened, providerBudget,
      ...(needsReplan ? { replanRequestedEffort } : {}),
      executionIntent: needsReplan ? executionIntent(null, entry.executionIntent.invocation) : entry.executionIntent };
    if (needsReplan && typeof opts.prepareTaskBody !== 'function') {
      fail('PROVIDER_REPLAN_REQUIRED', 'Changed model controls require a freshly validated execution contract.');
    }
    // Preparation precedes the durable approval mutation, so an invalid tuple
    // leaves the escalation pending rather than consuming its approval.
    const body = prepareBody(record, candidate, resolved.resolution.approver, 'delegation_escalation');
    if (needsReplan && (!body.execution || body.execution.provider !== entry.provider
      || body.execution.resolvedTaskTier !== widened.taskTier
      || body.execution.resolvedModelTier !== widened.model.modelTier
      || body.execution.requestedEffort !== replanRequestedEffort)) {
      fail('PROVIDER_REPLAN_REQUIRED', 'Changed model controls were not reproduced by the execution validator.');
    }
    entry.escalation = resolved;
...
module.exports = { createDelegationCoordinator, delegationTaskTier, DelegationError, ESCALATION_OUTCOMES };
