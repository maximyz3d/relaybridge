...
function createDelegationCoordinator(opts = {}) {
  ...
  const recordPath = (id) => {
    if (typeof id !== 'string' || id.length > 128 || id !== id.trim() || !/^dlg_[A-Za-z0-9_]+$/.test(id)) {
  ...
  function stats() {
    ...
    for (const name of fs.readdirSync(dir).filter((name) => name.startsWith('dlg_') && name.endsWith('.json'))) {
      const record = get(name.slice(0, -5));
      if (!record) continue;
      for (const entry of record.entries) {
