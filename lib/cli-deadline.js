...
function renderCliDeadline({ entry = {}, slot, supervisorOptions } = {}) {
  ...
  // No run is stopped because of elapsed time. The supervisor never enforces
  // a deadline any more, so the native CLI wait is rendered at the largest
  // value the flag can carry (Go int64-ns duration in ms, capped below the
  // safe-integer/ns overflow boundary) rather than a computed margin above a
  // ceiling that no longer exists.
  const cap = 2147483647;
  const printMs = cap;
  const duration = `${Math.floor(printMs / 1000)}s`;
  ...
  return { slot: rendered, deadline: { source: 'uncapped', hardCapMs: null,
    finite: false, renewable: false, nativeLimitMs: printMs, supervisorHardDeadline: false,
    limitation: 'native_cli_wait_cannot_be_renewed',
    printTimeoutMs: printMs, printTimeout: duration, marginMs: null } };
}
