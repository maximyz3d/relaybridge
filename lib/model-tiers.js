...
function applyModelArgs(slot, modelArgs, entry = {}) {
  if (!Array.isArray(slot) || !modelArgs || !modelArgs.length) return Array.isArray(slot) ? slot.slice() : [];
  const out = slot.slice();
  // Never inject a second --model if the slot already pins one.
  const flag = modelArgs[0];
  if (typeof flag === 'string' && flag.startsWith('-') && out.includes(flag)) return out;
