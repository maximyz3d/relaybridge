...
const { activeQualitativeQuotaExhaustion } = require('./vendor-quota');

function hasCapacityEvidence(gauge) {
  return !!gauge && ['vendor_observed', 'native_observed', 'operator_observed', 'metered'].includes(gauge.basis)
    && gauge.vendorQuota?.kind !== 'quota_exhausted';
}
...
function stressOf(gauge) {
  if (!hasCapacityEvidence(gauge)) return 0;
...
function suggestTierAdjustment({ tier, gauge, highStakes = false, explicitProvider = false }) {
  ...
  if (!hasCapacityEvidence(gauge) && gauge?.basis !== 'unmetered') {
    return { tier, changed: false, reason: 'remaining capacity unknown; configured estimates do not justify a downgrade' };
  }
...
function levelCandidates(candidates = [], gauges = {}, opts = {}) {
  ...
  const scored = candidates.map((c) => {
    ...
    else if (!hasCapacityEvidence(gauge)) {
      why.push('remaining capacity unknown; configured estimates excluded');
    } else if (Number.isFinite(gauge.percentRemaining)) {
      why.push(`${gauge.percentRemaining}% fuel (${gauge.basis})`);
    }
...
function fleetBalance(gauges = {}) {
  ...
  for (const gauge of Object.values(gauges)) {
    const quotaSeat = gauge.quotaSeat || gauge.seat;
    if (gauge.basis === 'unmetered') continue;
    if (!hasCapacityEvidence(gauge) || !Number.isFinite(gauge.percentRemaining)) {
      unknown.set(quotaSeat, { quotaSeat, basis: gauge.basis || 'unknown',
        aliases: Array.isArray(gauge.aliases) && gauge.aliases.length ? [...gauge.aliases] : [gauge.seat] });
      continue;
    }
    const prior = grouped.get(quotaSeat);
    // Duplicate aliases should be identical. If malformed input disagrees,
    // retain the conservative (lowest-fuel) view instead of calling it fresh.
    if (!prior || gauge.percentRemaining < prior.percentRemaining) {
      grouped.set(quotaSeat, { ...gauge, quotaSeat });
    }
  }
  const metered = [...grouped.values()];
  const unknownSeats = [...unknown.values()].filter((g) => !grouped.has(g.quotaSeat));
  const quotaSeats = metered.map((g) => ({
    quotaSeat: g.quotaSeat,
    aliases: Array.isArray(g.aliases) && g.aliases.length ? [...g.aliases] : [g.seat],
    percentRemaining: g.percentRemaining,
    basis: g.basis,
  }));
  if (metered.length < 2) {
    return { balanced: null, spread: null, advice: null, seats: metered.length, quotaSeats, unknownSeats };
  }
  ...
  return {
    ...
    mostDrained: lowest.quotaSeat,
    freshest: highest.quotaSeat,
    quotaSeats,
    unknownSeats,
    advice: spread > 20
      ? `shift work from ${lowest.quotaSeat} (${min}%) toward ${highest.quotaSeat} (${max}%) — ${spread}pt spread`
      : null,
  };
}
...
function applyCooldownsToDiagnostics(diagnostics = {}, cooling = [], explicitKinds = []) {
  ...
  for (const state of cooling || []) {
    const aliases = Array.isArray(state?.aliases) && state.aliases.length
      ? state.aliases.map(String) : [String(state?.seat || '')];
    for (const seat of aliases) {
      if (!seat) continue;
      ...
      out[seat] = {
        ...
        cooling: true,
        cooldown: state,
        detail: state.authority === 'unknown' ? 'cooldown authority unavailable; repair quota state before dispatch'
          : `quota seat ${state.quotaSeat || state.seat || seat} cooling after ${state.reason || 'quota failure'}; ${state.remainingSec || 0}s remaining`,
      };
      skipped.push(seat);
    }
  }
...
/**
 * Turn an active, vendor-observed exhausted quota into a hard routing gate.
 *
 * Cooldowns and bounded vendor quota observations both gate explicit requests.
 * Authoritative vendor exhaustion adds its own reset: retrying the same
 * seat before its reset cannot succeed, so even an explicitly preferred route
 * must be excluded. The usage ledger already resolves account-scoped evidence
 * across declared quota-seat aliases and model-scoped evidence only onto the
 * affected model. This function preserves that projection and adds a typed,
 * actionable reason to the route response.
 */
function activeVendorQuotaExhaustion(gauge, nowMs = Date.now()) {
  if (!gauge || gauge.basis !== 'vendor_observed' || !gauge.vendorQuota) return null;
  const quota = gauge.vendorQuota;
  if (quota.kind !== undefined) {
    const accepted = activeQualitativeQuotaExhaustion(quota, nowMs);
    if (!accepted) return null;
    const { kind: _observationKind, ...fields } = accepted;
    const resetMs = Date.parse(accepted.reset.expiresAt);
    return { reason:'vendor_quota_exhausted', authoritative:true, basis:'vendor_observed',
      quotaSeat:String(gauge.quotaSeat || gauge.seat || accepted.provider),
      aliases:Array.isArray(gauge.aliases) && gauge.aliases.length ? [...new Set(gauge.aliases.map(String))].sort() : [String(gauge.seat || accepted.provider)],
      ...fields, retry:{allowedAfter:accepted.reset.expiresAt,remainingMs:resetMs-nowMs,action:'use_alternate_quota_seat_until_reset'} };
  }
  const actual = Number(quota.actual);
  const limit = Number(quota.limit);
  const observedMs = Date.parse(quota.observedAt);
  const resetAt = quota.reset?.expiresAt;
  const resetMs = Date.parse(resetAt);
  if (!Number.isSafeInteger(actual) || actual < 0
    || !Number.isSafeInteger(limit) || limit < 1
    || !Number.isFinite(observedMs) || !Number.isFinite(resetMs)
    || resetMs <= observedMs || resetMs <= nowMs) return null;
  const exhausted = quota.overLimit === true || actual >= limit || gauge.remaining === 0;
  if (!exhausted) return null;
  const aliases = Array.isArray(gauge.aliases) && gauge.aliases.length
    ? [...new Set(gauge.aliases.map(String))].sort() : [String(gauge.seat || quota.provider)];
  return {
    reason: 'vendor_quota_exhausted',
    authoritative: true,
    basis: 'vendor_observed',
    quotaSeat: String(gauge.quotaSeat || gauge.seat || quota.provider),
    aliases,
    scope: quota.scope === 'account' ? 'account' : 'model',
    provider: String(quota.provider || gauge.seat || ''),
    model: quota.model || null,
    actual,
    limit,
    remaining: Math.max(0, limit - actual),
    overLimit: quota.overLimit === true || actual > limit,
    observedAt: new Date(observedMs).toISOString(),
    reset: {
      kind: quota.reset?.kind || 'vendor_reset',
      expiresAt: new Date(resetMs).toISOString(),
      note: quota.reset?.note || null,
    },
    retry: {
      allowedAfter: new Date(resetMs).toISOString(),
      remainingMs: Math.max(0, resetMs - nowMs),
      action: 'use_alternate_quota_seat_until_reset',
    },
  };
}

function applyVendorQuotaExhaustionToDiagnostics(diagnostics = {}, gauges = {}, opts = {}) {
  ...
  for (const [seat, gauge] of Object.entries(gauges || {})) {
    const quota = activeVendorQuotaExhaustion(gauge, nowMs);
    if (!quota) continue;
    const prior = out[seat] && typeof out[seat] === 'object' ? out[seat] : {};
    const item = { kind: seat, blocked: true, ...quota };
    out[seat] = {
      ...prior,
      ready: false,
      vendorQuotaExhausted: true,
      vendorQuotaBlock: item,
      detail: `vendor quota exhausted for ${quota.quotaSeat}; retry after ${quota.retry.allowedAfter}`,
    };
    skipped.push(item);
  }
  return { diagnostics: out, skipped };
...
module.exports = {
  ...
  applyVendorQuotaExhaustionToDiagnostics, levelRouteSelection,
};
