  ...
  for (const state of cooling || []) {
    ...
    for (const seat of aliases) {
      ...
      out[seat] = {
        ...
        cooling: true,
        cooldown: state,
        detail: `quota seat ${state.quotaSeat || state.seat || seat} cooling after ${state.reason || 'quota failure'}; ${state.remainingSec || 0}s remaining`,
      };
      skipped.push(seat);
    }
  }
 ...
 *
 * Cooldowns are heuristic backoff and an explicit provider request may bypass
 * them. A bounded vendor quota observation is different: retrying the same
