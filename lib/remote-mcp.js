...
function bearerFrom(req) {
  ...
  if (typeof auth === 'string' && auth.length <= 8192
    && auth.slice(0, 6).toLowerCase() === 'bearer'
    && (auth[6] === ' ' || auth[6] === '\t')
    && !/[\r\n\u2028\u2029]/.test(auth)) {
    return auth.slice(7).trim();
  }
  // Connector UIs that only offer a custom header still work.
  const custom = req.headers?.['x-relaybridge-token'];
  return typeof custom === 'string' && custom.length <= 8192 ? custom : '';
}
...
function mountRemoteMcp(app, opts) {
  ...
  const factory = () => {
    ...
    try {
      ...
      // The SDK can finish an HTTP exchange before a cancelled tool callback
      // settles. Retain actual callback lifetime independently of transport.
      if (opts.wrapCallback) {
        for (const [registry, field, tool] of [
          [server._registeredTools || server.registeredTools, 'handler', true],
          [server._registeredResources || server.registeredResources, 'readCallback'],
          [server._registeredResourceTemplates, 'readCallback'],
          [server._registeredPrompts, 'handler'],
        ]) {
          const names = registryNames(registry);
          if (names === null) throw new Error('MCP callback registry cannot be supervised');
          for (const name of names) {
            const entry = registry[name];
            if (typeof entry?.[field] !== 'function') {
              throw new Error('MCP callback lifetime cannot be supervised');
            }
            const wrapped = opts.wrapCallback(entry[field]);
            if (tool) {
              // SDK 2 caches an executor closure; updating only .handler
              // would leave the originally registered callback in service.
              if (typeof entry.update !== 'function') throw new Error('MCP tool executor cannot be supervised');
              entry.update({callback:wrapped});
              if (entry.handler !== wrapped) throw new Error('MCP tool callback supervision did not take effect');
            } else entry[field] = wrapped;
          }
        }
      }
    } catch (error) {
  ...
  const wrapHandler = opts.wrapHandler || (handler => handler);
  app.all(mountPath, wrapHandler(async (req, res) => {
