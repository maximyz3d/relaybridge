...
function mountRemoteMcp(app, opts) {
  ...
  const factory = () => {
    ...
    const resources = server._registeredResources || server.registeredResources;
    if (resources && typeof resources === 'object') {
      for (const name of Object.keys(resources)) if (isNeverRemoteResource(name)) delete resources[name];
    }
    return server;
  };
