...
async function onboardRepo(opts) {
  ...
  if (!cwd || !fs.existsSync(path.join(cwd, '.git'))) {
    cwd = path.join(require('os').tmpdir(), 'rb-onboard', opts.name.replace('/', '__'));
    fs.mkdirSync(cwd, { recursive: true });
    if (!fs.existsSync(path.join(cwd, '.git'))) {
      const clone = await gh(['repo', 'clone', opts.name, cwd, '--', '--depth', '5']);
  ...
  if (fs.existsSync(labelsFile)) {
    ...
    for (const l of labels) {
      const r = await gh(['label', 'create', l.name, '--repo', opts.name, '--color', l.color || 'ededed', '--description', l.description || '', '--force']);
      (r.code === 0 ? report.installed : report.warnings).push(`label ${l.name}` + (r.code === 0 ? '' : `: ${r.stderr.trim().slice(0, 120)}`));
    }
  }
