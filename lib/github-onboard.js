...
async function onboardRepo(opts) {
  ...
  // 2. Locate or clone; branch off default. Never switch branches in a caller's
  // existing checkout: it may contain unrelated work or be actively used by an
  // IDE. A supplied checkout is registry context only; onboarding happens in a
  // fresh temporary clone of the named GitHub repository.
  const registryPath = opts.path;
  let cwd = null;
  let disposableClone = false;
  if (registryPath && fs.existsSync(path.join(registryPath, '.git'))) {
    const tempParent = fs.mkdtempSync(path.join(os.tmpdir(), 'rb-onboard-work-'));
    cwd = path.join(tempParent, opts.name.replace('/', '__'));
  ...
  if (!registry.repos.some((r) => r.name === opts.name)) {
    registry.repos.push(tracker.defaultRepoEntry({ name: opts.name, path: registryPath || cwd }));
  ...
  if (disposableClone) {
    try { fs.rmSync(path.dirname(cwd), { recursive: true, force: true }); }
    catch (err) { result.warnings.push(`temporary clone cleanup failed: ${err.message}`); }
  }
  return result;
}
