...
function normalizeQualitativeQuotaExhaustion(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)
    ...
    || value.kind !== 'quota_exhausted' || value.provider !== 'gemini' || value.scope !== 'account'
    || value.source !== QUALITATIVE_SOURCE || !['stderr','stdout'].includes(value.diagnosticSource)
    || value.seat !== undefined && value.seat !== 'gemini'
  ...
  return { kind:'quota_exhausted', provider:'gemini', model:value.model, scope:'account', unit:null,
    actual:null, limit:null, remaining:null, percentRemaining:null, overLimit:null,
    source:QUALITATIVE_SOURCE, diagnosticSource:value.diagnosticSource, observedAt:value.observedAt,
...
function parseGeminiQuotaExhaustion({provider,stdout='',stderr='',exitCode,stopReason,supervisorStopReason,model=null,observedAt=new Date()} = {}) {
  ...
  const match = !out ? err.match(pattern) : !err ? out.match(pattern) : null;
