...
function clip(value, limit, kind = 'phase prompt') {
  return requireCompleteText(cleanText(value), limit, kind);
}

function list(value, limit = 40) {
  const values = Array.isArray(value) ? value : value == null ? [] : [value];
  if (values.length > limit) {
    const error = new Error(`instruction list exceeds ${limit} entries; supply a complete bounded packet`);
...
function section(title, value, limit) {
  return `## ${title}\n\n${clip(value, limit, title) || 'None supplied.'}`;
}
...
// Keep delivery/tool guidance in the reserved suffix, not clipped artifacts.
function finalDeliveryContract(blockingMarker, { readOnly = false, provider = 'claude' } = {}) {
  return [
    'Deliver the entire requested artifact and its status marker together in one self-contained final response. Only that final response is handed to the next phase; earlier assistant messages are not the deliverable. Do not substitute a postscript such as "the plan above is complete", refer the reader to an earlier message, or send a separate closing note after the artifact.',
    ...(readOnly ? [
      provider === 'codex'
        ? 'This is a headless read-only phase. Use bounded file reads and read-only inspection commands only. Do not write files, run tests that produce artifacts, launch providers or other agents, or invoke RelayBridge. Return the artifact inline in the final response.'
        : 'This is a headless read-only phase: use Read/Glob/Grep only. Do not use Write, Edit, Bash, or ExitPlanMode; do not create a plan file or wait for a plan-mode approval dialog. Return the artifact inline in the final response.',
    ] : []),
    `If evidence required for this phase is inaccessible or missing, identify it and the smallest needed handoff, then end with ${blockingMarker}. Do not invent evidence or approval. Distinguish an essential evidence blocker from optional verification that was not run; report optional gaps without treating them as automatic blockers.`,
  ].join('\n\n');
}
...
function buildPlanningPrompt(input = {}) {
  const body = [
    input.provider === 'codex'
      ? 'You are a fresh Codex planning specialist. The root writer has scoped and researched the objective. Inspect the workspace using bounded read-only commands where evidence is missing. Produce the implementation plan only; do not edit files or perform implementation.'
  ...
  const outputContract = [
    ...
    '',
    finalDeliveryContract('PLAN_STATUS: BLOCKED', { readOnly: true, provider: input.provider }),
    '',
...
function buildReviewPrompt(input = {}, { final = false } = {}) {
  const body = [
    input.provider === 'codex'
      ? `You are a fresh Codex ${final ? 'final ' : ''}reviewer. Independently inspect the implementation against the objective and plan using bounded read-only commands. Check correctness, regressions, security, missing tests and unnecessary scope. Do not edit files, execute implementation, or rely on an earlier reviewer's conclusion.`
      : final
  ...
  const outputContract = [
    ...
    '',
    finalDeliveryContract('REVIEW_VERDICT: BLOCK', { readOnly: true, provider: input.provider }),
    '',
    'End with exactly one marker on its own line:',
...
function buildRevisionPrompt(input = {}) {
  ...
  const outputContract = [
    ...
    '',
    finalDeliveryContract('REVISION_STATUS: BLOCKED'),
    '',
    'End with exactly one marker on its own line:',
