...
function finalDeliveryContract(blockingMarker, { readOnly = false } = {}) {
  return [
...
function buildPlanningPrompt(input = {}) {
  ...
  const outputContract = [
    ...
    '',
    finalDeliveryContract('PLAN_STATUS: BLOCKED', { readOnly: true }),
    '',
...
function buildReviewPrompt(input = {}, { final = false } = {}) {
  ...
  const outputContract = [
    ...
    '',
    finalDeliveryContract('REVIEW_VERDICT: BLOCK', { readOnly: true }),
    '',
    'End with exactly one marker on its own line:',
