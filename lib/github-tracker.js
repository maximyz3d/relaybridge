...
// repo (data/github-repos.json by default); it is a strict no-op otherwise. After a
...
const LEGACY_REGISTRY_PARTS = ['config', 'github-repos.json'];

// Registry location precedence is intentionally centralized here so tracking,
// onboarding, fleet upgrades, the REST API, and installers all resolve the
// same durable file:
//
//   1. RELAYBRIDGE_GITHUB_REPOS (an explicit operator override)
//   2. <RELAYBRIDGE_DATA_DIR>/github-repos.json
//   3. <PS_BRIDGE_DATA_DIR>/github-repos.json (legacy env alias)
//   4. <checkout>/data/github-repos.json
//
// config/github-repos.json was the pre-2.1 location. It is a migration source,
// never a live fallback: after a runtime registry exists, even a malformed
// legacy file is ignored and can never overwrite current enrollment.
function registryPaths(options = {}) {
  const env = options.env || process.env;
  const detected = options.platform || platform.detectPlatform();
  const pathApi = detected.isWindows ? path.win32 : path.posix;
  const resolveConfiguredPath = (value, fallback, label) => {
    const configured = value ? String(value) : '';
    const windowsUncOnPosix = !detected.isWindows
      && /^(?:\\\\|\/\/)[^\\/]+[\\/][^\\/]+/.test(configured);
    if (configured && (!pathApi.isAbsolute(configured) || windowsUncOnPosix)) {
      throw new Error(`${label} must be an absolute path for ${detected.label || (detected.isWindows ? 'Windows' : 'POSIX')}: ${JSON.stringify(configured)}`);
    }
    return pathApi.resolve(configured || fallback);
  };
  const root = resolveConfiguredPath(options.root, ROOT, 'RelayBridge root');
  const dataDir = resolveConfiguredPath(
    env.RELAYBRIDGE_DATA_DIR || env.PS_BRIDGE_DATA_DIR,
    pathApi.join(root, 'data'),
    'GitHub registry data directory',
  );
  const runtimeFile = resolveConfiguredPath(
    options.runtimeFile || env.RELAYBRIDGE_GITHUB_REPOS,
    pathApi.join(dataDir, 'github-repos.json'),
    'GitHub registry file',
  );
  const legacyFile = resolveConfiguredPath(
    options.legacyFile,
    pathApi.join(root, ...LEGACY_REGISTRY_PARTS),
    'legacy GitHub registry file',
  );
  const allowSlow = options.allowSlow === true
    || /^(1|true|yes)$/i.test(String(env.RELAYBRIDGE_ALLOW_SLOW_WSL_FS || ''));
  const portableRuntime = runtimeFile.replace(/\\/g, '/');
  const slowWslRuntime = portableRuntime === '/mnt' || portableRuntime.startsWith('/mnt/')
    || platform.isSlowWslInteropPath(runtimeFile);
  if (detected.isWSL && slowWslRuntime && !allowSlow) {
    throw new Error(`GitHub registry must use the WSL Linux filesystem, not /mnt: ${runtimeFile}`);
  }
  return {
    root,
    dataDir,
    runtimeFile,
    legacyFile,
    exampleFile: pathApi.join(root, 'config', 'github-repos.example.json'),
    explicit: !!(options.runtimeFile || env.RELAYBRIDGE_GITHUB_REPOS),
  };
}

const REGISTRY_FILE = registryPaths().runtimeFile;
const LEGACY_REGISTRY_FILE = registryPaths().legacyFile;
...
function normalizeRepoPath(value, options = {}) {
  const detected = options.platform || platform.detectPlatform();
  const input = String(value || '').trim();
  if (!input) throw new Error('local path is empty');

  const pathApi = detected.isWindows ? path.win32 : path.posix;
  if (!pathApi.isAbsolute(input)) {
    throw new Error(`local path must be absolute on ${detected.label || detected.os || 'this host'}: ${JSON.stringify(input)}`);
  }
  if (detected.isWindows
      && !/^[A-Za-z]:[\\/]/.test(input)
      && !/^\\\\[^\\]+\\[^\\]+/.test(input)) {
    throw new Error(`Windows local path must include a drive or UNC share: ${JSON.stringify(input)}`);
  }
  let normalized = pathApi.normalize(input);

  // Windows drive paths are not Linux paths. Without this check, path.resolve
  // on WSL turns C:\\work\\repo into a plausible-looking child of the bridge
  // checkout and tracking silently no-ops forever.
  if (!detected.isWindows && /^[A-Za-z]:[\\/]/.test(normalized)) {
    throw new Error(`Windows path cannot be used by the ${detected.label || 'POSIX'} bridge: ${JSON.stringify(input)}`);
  }
  // Store a canonical native path when it exists, while allowing enrollment
  // before a checkout is created. realpathSync.native uses the host's native
  // representation and avoids preserving a slow or misleading symlink alias.
  if (!options.skipRealpath) {
    try { normalized = fs.realpathSync.native(normalized); } catch {}
  }
  if (detected.isWSL) {
    const slashPath = normalized.replace(/\\/g, '/');
    const slow = slashPath === '/mnt' || slashPath.startsWith('/mnt/')
      || platform.isSlowWslInteropPath(normalized);
    const allowSlow = options.allowSlow === true
      || /^(1|true|yes)$/i.test(String((options.env || process.env).RELAYBRIDGE_ALLOW_SLOW_WSL_FS || ''));
    if (slow && !allowSlow) {
      throw new Error(`WSL repo path must use the Linux filesystem, not /mnt: ${JSON.stringify(input)}`);
    }
  }
  return normalized;
}

function parseRegistry(raw, source = 'github-repos.json', options = {}) {
  let parsed;
  try { parsed = JSON.parse(String(raw)); } catch (err) {
    throw new Error(`${source} is not valid JSON: ${err.message}`);
  }
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed) || !Array.isArray(parsed.repos)) {
    throw new Error(`${source} must contain a repos array`);
  }
  const repos = parsed.repos.map((r, index) => {
    if (!r || typeof r !== 'object' || Array.isArray(r)) {
      throw new Error(`registry entry ${index} must be an object`);
    }
    return defaultRepoEntry(r);
  });
  for (const repo of repos) {
    ...
    try { repo.path = normalizeRepoPath(repo.path, options); }
    catch (err) { throw new Error(`registry entry ${repo.name} has invalid local path: ${err.message}`); }
...
function writePrivateTemp(target, contents, purpose) {
  fs.mkdirSync(path.dirname(target), { recursive: true });
  const temp = `${target}.${purpose}.${process.pid}.${Date.now().toString(36)}.${Math.random().toString(16).slice(2)}.tmp`;
  let fd;
  try {
...
// Persist the directory entry created by link/rename when the host filesystem
// supports directory fsync. Windows and some Unix filesystems reject opening or
// syncing a directory; publication has already succeeded in that case, so this
// remains deliberately best-effort instead of turning a supported atomic write
// into a platform-specific startup failure.
function fsyncParentDirectory(target) {
  let fd;
  try {
    const flags = fs.constants.O_RDONLY | (fs.constants.O_DIRECTORY || 0);
    fd = fs.openSync(path.dirname(target), flags);
    fs.fsyncSync(fd);
    return true;
  } catch {
    return false;
...
// Enrollment is authority state. Inspect the directory entry without following
// it, then read only the exact regular-file descriptor whose identity was
// inspected. This primitive deliberately returns bytes without parsing so the
// same confinement applies to both current runtime state and the legacy input
// an installer may migrate. Only ENOENT from the initial lstat means "missing";
// every disappearance or replacement after that point fails closed.
function readRegistryBytesIfPresent(target, options = {}, label = 'GitHub registry') {
  const fsApi = options.fsApi || fs;
  let before;
  try { before = fsApi.lstatSync(target, { bigint: true }); }
  catch (err) {
    if (err && err.code === 'ENOENT') return null;
    throw new Error(`cannot inspect ${label} ${target}: ${err.message}`);
  }
  if (!before.isFile()) {
    const kind = before.isSymbolicLink() ? 'symbolic links are not allowed' : 'the target is not a regular file';
    throw new Error(`${label} ${target} must be a regular file; ${kind}`);
  }

  let fd;
  try {
    const isPosix = process.platform !== 'win32';
    const noFollow = fsApi.constants.O_NOFOLLOW;
    const nonBlock = fsApi.constants.O_NONBLOCK;
    if (isPosix && (!Number.isInteger(noFollow) || noFollow === 0
        || !Number.isInteger(nonBlock) || nonBlock === 0)) {
      throw new Error('this POSIX runtime does not expose O_NOFOLLOW and O_NONBLOCK');
    }
    // O_NONBLOCK prevents a regular-file-to-FIFO replacement from blocking
    // the bridge between lstat and fstat. O_NOFOLLOW makes a symlink swap fail
    // in open itself on POSIX. Windows lacks O_NOFOLLOW, so the descriptor and
    // a second lstat are still identity-checked before any bytes are read.
    const flags = fsApi.constants.O_RDONLY
      | (isPosix ? noFollow | nonBlock | (fsApi.constants.O_NOCTTY || 0) : 0);
    fd = fsApi.openSync(target, flags);
    const opened = fsApi.fstatSync(fd, { bigint: true });
    if (!opened.isFile()) {
      throw new Error('the opened target is not a regular file');
    }
    if (typeof before.dev !== 'bigint' || typeof before.ino !== 'bigint'
        || typeof opened.dev !== 'bigint' || typeof opened.ino !== 'bigint'
        || before.dev === 0n || before.ino === 0n
        || opened.dev === 0n || opened.ino === 0n) {
      throw new Error('the filesystem did not provide a stable registry identity');
    }
    if (opened.dev !== before.dev || opened.ino !== before.ino) {
      throw new Error('the registry target changed before it could be opened');
    }
    const after = fsApi.lstatSync(target, { bigint: true });
    if (!after.isFile()
        || after.dev !== opened.dev || after.ino !== opened.ino) {
      throw new Error('the registry target changed before it could be read');
    }
    const raw = fsApi.readFileSync(fd);
    return Buffer.isBuffer(raw) ? raw : Buffer.from(raw);
  } catch (err) {
    throw new Error(`cannot securely read ${label} ${target}: ${err.message}`);
  } finally {
    if (fd !== undefined) try { fsApi.closeSync(fd); } catch {}
  }
}

function readRuntimeRegistryIfPresent(target, options = {}) {
  const raw = readRegistryBytesIfPresent(target, options);
  return raw === null ? null : parseRegistry(raw.toString('utf8'), target, options);
}

function migrateLegacyRegistry(options = {}) {
  const paths = registryPaths(options);
  if (readRuntimeRegistryIfPresent(paths.runtimeFile, options) !== null) {
    return { status: 'runtime-present', runtimeFile: paths.runtimeFile, legacyFile: paths.legacyFile };
  }
  const raw = readRegistryBytesIfPresent(paths.legacyFile, options, 'legacy GitHub registry');
  if (raw === null) {
    return { status: 'legacy-missing', runtimeFile: paths.runtimeFile, legacyFile: paths.legacyFile };
  }

  try { parseRegistry(raw.toString('utf8'), `legacy GitHub registry ${paths.legacyFile}`, options); }
  catch (err) { throw new Error(`${err.message}; it was not migrated`); }

  const contents = raw.length > 0 && raw[raw.length - 1] === 0x0a
    ? raw
    : Buffer.concat([raw, Buffer.from('\n')]);
  const temp = writePrivateTemp(paths.runtimeFile, contents, 'migrate');
  try {
    // A same-directory hard link atomically publishes the already-fsynced
    // bytes and fails with EEXIST if another bridge/onboarder won the race.
    // Unlike rename(), this can never replace an existing runtime registry on
    // POSIX. It has the same no-replace behavior on NTFS.
    try {
      fs.linkSync(temp, paths.runtimeFile);
    } catch (err) {
      if (err && err.code === 'EEXIST') {
        if (readRuntimeRegistryIfPresent(paths.runtimeFile, options) === null) {
          throw new Error(`GitHub registry race winner disappeared before validation: ${paths.runtimeFile}`);
        }
        return { status: 'runtime-present', runtimeFile: paths.runtimeFile, legacyFile: paths.legacyFile };
      }
      throw err;
    }
    fsyncParentDirectory(paths.runtimeFile);
    return { status: 'migrated', runtimeFile: paths.runtimeFile, legacyFile: paths.legacyFile };
  } finally {
    try { fs.unlinkSync(temp); } catch {}
  }
}

function loadRegistry(file, options = {}) {
  if (!file && options.migrate === false) {
    const paths = registryPaths(options);
    return readRuntimeRegistryIfPresent(paths.runtimeFile, options)
      ?? readRuntimeRegistryIfPresent(paths.legacyFile, options) ?? { repos: [] };
  }
  const migration = file ? null : migrateLegacyRegistry(options);
  const target = file
    ? registryPaths({ ...options, runtimeFile: file }).runtimeFile
    : migration.runtimeFile;
  const loaded = readRuntimeRegistryIfPresent(target, options);
  if (loaded !== null) return loaded;
  if (migration && migration.status !== 'legacy-missing') {
    throw new Error(`GitHub registry disappeared before it could be loaded: ${target}`);
  }
  return { repos: [] };
}

function saveRegistry(registry, file, options = {}) {
  const target = file
    ? registryPaths({ ...options, runtimeFile: file }).runtimeFile
    : registryPaths(options).runtimeFile;
  const validated = parseRegistry(JSON.stringify(registry), target, options);
  const contents = JSON.stringify(validated, null, 2) + '\n';
  const temp = writePrivateTemp(target, contents, 'save');
  try {
...
module.exports = {
  ...
  registryPaths, normalizeRepoPath, parseRegistry, migrateLegacyRegistry,
  defaultRepoEntry, loadRegistry, saveRegistry, fsyncParentDirectory, repoForCwd,
