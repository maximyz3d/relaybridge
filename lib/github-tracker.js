...
function registryPaths(options = {}) {
  ...
  const root = pathApi.resolve(options.root || ROOT);
...
// Runtime enrollment is authority state. Inspect the directory entry without
// following it, then parse only a regular file. In particular, a dangling
// symlink must never look like an absent registry and suppress valid legacy
// enrollment, while a live symlink must never redirect reads outside the
...
function readRuntimeRegistryIfPresent(target, options = {}) {
  ...
  try { stat = fs.lstatSync(target); }
    ...
    throw new Error(`cannot inspect GitHub registry ${target}: ${err.message}`);
  }
  if (!stat.isFile()) {
    const kind = stat.isSymbolicLink() ? 'symbolic links are not allowed' : 'the target is not a regular file';
    throw new Error(`GitHub registry ${target} must be a regular file; ${kind}`);
  }
  let raw;
  try { raw = fs.readFileSync(target, 'utf8'); }
  catch (err) { throw new Error(`cannot read GitHub registry ${target}: ${err.message}`); }
  return parseRegistry(raw, target, options);
}
...
function migrateLegacyRegistry(options = {}) {
  ...
  if (!fs.existsSync(paths.legacyFile)) {
    return { status: 'legacy-missing', runtimeFile: paths.runtimeFile, legacyFile: paths.legacyFile };
  }

  let raw;
  try { raw = fs.readFileSync(paths.legacyFile, 'utf8'); }
  catch (err) { throw new Error(`cannot read legacy GitHub registry ${paths.legacyFile}: ${err.message}`); }
  try { parseRegistry(raw, `legacy GitHub registry ${paths.legacyFile}`, options); }
  catch (err) { throw new Error(`${err.message}; it was not migrated`); }

  const temp = writePrivateTemp(paths.runtimeFile, raw.endsWith('\n') ? raw : `${raw}\n`, 'migrate');
  try {
    ...
    try { fs.chmodSync(paths.runtimeFile, 0o600); } catch {}
